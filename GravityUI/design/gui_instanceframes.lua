...
local function StyleDropdown(dropdown, sr, sg, sb, sa, bgr, bgg, bgb, bga, width)
    if not dropdown or dropdown.guiStyled then return end
    ...
    -- Create backdrop
    if not dropdown.guiBackdrop then
        dropdown.guiBackdrop = CreateFrame("Frame", nil, dropdown, "BackdropTemplate")
        dropdown.guiBackdrop:SetPoint("TOPLEFT", 0, -2)
        dropdown.guiBackdrop:SetPoint("BOTTOMRIGHT", 0, 2)
        dropdown.guiBackdrop:SetFrameLevel(dropdown:GetFrameLevel())
        dropdown.guiBackdrop:EnableMouse(false)
    end

    dropdown.guiBackdrop:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        edgeFile = "Interface\\Buttons\\WHITE8x8",
        edgeSize = 1,
        insets = { left = 1, right = 1, top = 1, bottom = 1 }
    ...
    local btnBgG = math.min(bgg + 0.07, 1)
    local btnBgB = math.min(bgb + 0.07, 1)
    dropdown.guiBackdrop:SetBackdropColor(btnBgR, btnBgG, btnBgB, 1)
    dropdown.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)

    -- Store colors for hover
    dropdown.guiSkinColor = { sr, sg, sb, sa }
    ...
    dropdown.guiStyled = true
end

-- Style tab button
local function StyleTab(tab, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not tab or tab.guiStyled then return end
    ...
    if not tab.guiBackdrop then
        ...
        tab.guiBackdrop:SetFrameLevel(tab:GetFrameLevel())
        tab.guiBackdrop:EnableMouse(false)
    end
    ...
    tab.guiBackdrop:SetBackdropColor(bgr, bgg, bgb, 0.9)
...
local function StyleGroupFinderButton(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if button.icon then
        ...
        -- Add icon border
        if not button.icon.guiBackdrop then
            button.icon.guiBackdrop = CreateFrame("Frame", nil, button, "BackdropTemplate")
            button.icon.guiBackdrop:SetPoint("TOPLEFT", button.icon, -1, 1)
            button.icon.guiBackdrop:SetPoint("BOTTOMRIGHT", button.icon, 1, -1)
            button.icon.guiBackdrop:SetFrameLevel(button:GetFrameLevel())
            button.icon.guiBackdrop:EnableMouse(false)
            button.icon.guiBackdrop:SetBackdrop({
                bgFile = nil,
                edgeFile = "Interface\\Buttons\\WHITE8x8",
                edgeSize = 1,
            })
            button.icon.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        end
    end
...
-- Style close button (gui pattern - minimal, just hide border)
...
local function SkinPVEFrame()
    local PVEFrame = _G.PVEFrame
    if not PVEFrame or PVEFrame.guiSkinned then return end
    ...
    CreateguiBackdrop(PVEFrame, sr, sg, sb, sa, bgr, bgg, bgb, bga)

    -- Style close button (minimal - just hide border per gui pattern)
    ...
    -- gui: Tab1 at x=-3, tabs at -5px spacing
    ...
    -- Style GroupFinder buttons (following ElvUI pattern)
...
local function SkinLFDFrame()
    local LFDQueueFrame = _G.LFDQueueFrame
    if not LFDQueueFrame or LFDQueueFrame.guiSkinned then return end
    ...
    LFDQueueFrame.guiSkinned = true
end

-- Skin RaidFinderQueueFrame (Raid Finder)
local function SkinRaidFinderFrame()
    local RaidFinderQueueFrame = _G.RaidFinderQueueFrame
    if not RaidFinderQueueFrame or RaidFinderQueueFrame.guiSkinned then return end
    ...
    RaidFinderQueueFrame.guiSkinned = true
end
...
local function SkinLFGListFrame()
    local LFGListFrame = _G.LFGListFrame
    if not LFGListFrame or LFGListFrame.guiSkinned then return end
    ...
    if LFGListFrame.CategorySelection then
        ...
        if cs.CategoryButtons then
            for _, catButton in pairs(cs.CategoryButtons) do
                if catButton and not catButton.guiStyled then
    ...
    if LFGListFrame.SearchPanel then
        ...
        if sp.SearchBox then
            ...
            CreateguiBackdrop(sp.SearchBox, sr, sg, sb, sa, bgr, bgg, bgb, bga)
        end
        -- Style filter button
...
local function StyleDungeonIcon(icon, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not icon or icon.guiStyled then return end
    ...
    if icon.Icon then
        ...
        if not icon.Icon.guiBackdrop then
            icon.Icon.guiBackdrop = CreateFrame("Frame", nil, icon, "BackdropTemplate")
            icon.Icon.guiBackdrop:SetPoint("TOPLEFT", icon.Icon, -1, 1)
            icon.Icon.guiBackdrop:SetPoint("BOTTOMRIGHT", icon.Icon, 1, -1)
            icon.Icon.guiBackdrop:SetFrameLevel(icon:GetFrameLevel())
            icon.Icon.guiBackdrop:EnableMouse(false)
            icon.Icon.guiBackdrop:SetBackdrop({
                bgFile = nil,
                edgeFile = "Interface\\Buttons\\WHITE8x8",
                edgeSize = 1,
            })
            icon.Icon.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        end
    end

    -- Store colors
    icon.guiSkinColor = { sr, sg, sb, sa }
...
local function StyleAffixIcon(affix, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not affix or affix.guiStyled then return end
    ...
    if affix.Portrait then
        ...
        if not affix.Portrait.guiBackdrop then
            affix.Portrait.guiBackdrop = CreateFrame("Frame", nil, affix, "BackdropTemplate")
            affix.Portrait.guiBackdrop:SetPoint("TOPLEFT", affix.Portrait, -1, 1)
            affix.Portrait.guiBackdrop:SetPoint("BOTTOMRIGHT", affix.Portrait, 1, -1)
            affix.Portrait.guiBackdrop:SetFrameLevel(affix:GetFrameLevel())
            affix.Portrait.guiBackdrop:EnableMouse(false)
            affix.Portrait.guiBackdrop:SetBackdrop({
                bgFile = nil,
                edgeFile = "Interface\\Buttons\\WHITE8x8",
                edgeSize = 1,
            })
            affix.Portrait.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        end
    end

    affix.guiStyled = true
end

-- Skin ChallengesFrame (M+ Dungeons tab)
local function SkinChallengesFrame()
    local ChallengesFrame = _G.ChallengesFrame
    if not ChallengesFrame or ChallengesFrame.guiSkinned then return end
    ...
    if ChallengesFrame.Update and not ChallengesFrame.guiUpdateHooked then
        ...
        ChallengesFrame.guiUpdateHooked = true
    end
    ...
    ChallengesFrame.guiSkinned = true
end
...
local function StylePVPActivityButton(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if button.Reward then
        ...
        if reward.Icon then
            ...
            if not reward.Icon.guiBackdrop then
                reward.Icon.guiBackdrop = CreateFrame("Frame", nil, reward, "BackdropTemplate")
                reward.Icon.guiBackdrop:SetPoint("TOPLEFT", reward.Icon, -1, 1)
                reward.Icon.guiBackdrop:SetPoint("BOTTOMRIGHT", reward.Icon, 1, -1)
                reward.Icon.guiBackdrop:SetFrameLevel(reward:GetFrameLevel())
                reward.Icon.guiBackdrop:EnableMouse(false)
                reward.Icon.guiBackdrop:SetBackdrop({
                    bgFile = nil,
                    edgeFile = "Interface\\Buttons\\WHITE8x8",
                    edgeSize = 1,
                })
                reward.Icon.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
            end
        end
    end
...
local function StylePVPRoleIcon(roleIcon, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not roleIcon or roleIcon.guiStyled then return end
...
local function StyleSpecificBGButton(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    button.guiBackdrop:SetBackdropColor(btnBgR, btnBgG, btnBgB, 0.9)
    button.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    ...
    button:HookScript("OnEnter", function(self)
        if self.guiBackdrop then
            self.guiBackdrop:SetBackdropBorderColor(1, 1, 1, 1)
        end
    end)
    button:HookScript("OnLeave", function(self)
        if self.guiBackdrop then
            self.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        end
    end)
...
local function StyleConquestBar(bar, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not bar or bar.guiStyled then return end
    ...
    if not bar.guiBackdrop then
        ...
        bar.guiBackdrop:SetFrameLevel(bar:GetFrameLevel())
    ...
    bar.guiBackdrop:SetBackdropColor(bgr, bgg, bgb, 0.8)
...
local function SkinPVPFrame()
    local PVPQueueFrame = _G.PVPQueueFrame
    if not PVPQueueFrame or PVPQueueFrame.guiSkinned then return end
    ...
    if HonorFrame then
        ...
        if HonorFrame.SpecificScrollBox and not HonorFrame.SpecificScrollBox.guiHooked then
            ...
            HonorFrame.SpecificScrollBox.guiHooked = true
        end
    ...
    if TrainingGroundsFrame then
        ...
        if specificList and specificList.ScrollBox and not specificList.ScrollBox.guiHooked then
            -- Hook to style buttons as they're created/recycled
            ...
            specificList.ScrollBox.guiHooked = true
        ...
        TrainingGroundsFrame.guiSkinned = true
    end
    ...
    if PlunderstormFrame then
        ...
        PlunderstormFrame.guiSkinned = true
    end

    PVPQueueFrame.guiSkinned = true
end
...
local function UpdateTabColors(tab, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not tab or not tab.guiBackdrop then return end
    tab.guiBackdrop:SetBackdropColor(bgr, bgg, bgb, 0.9)
    tab.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
end
...
local function UpdateGroupFinderButtonColors(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if button.icon and button.icon.guiBackdrop then
        button.icon.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
end
...
local function UpdatePVPActivityButtonColors(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if button.Reward and button.Reward.Icon and button.Reward.Icon.guiBackdrop then
        button.Reward.Icon.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
end

-- Helper to update conquest bar colors
local function UpdateConquestBarColors(bar, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not bar or not bar.guiBackdrop then return end
    bar.guiBackdrop:SetBackdropColor(bgr, bgg, bgb, 0.8)
    bar.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
end
...
local function UpdateDungeonIconColors(icon, sr, sg, sb, sa)
    if not icon or not icon.Icon or not icon.Icon.guiBackdrop then return end
    icon.Icon.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    icon.guiSkinColor = { sr, sg, sb, sa }
end
...
local function UpdateAffixIconColors(affix, sr, sg, sb, sa)
    if not affix or not affix.Portrait or not affix.Portrait.guiBackdrop then return end
    affix.Portrait.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
end

-- Helper to update dropdown colors
local function UpdateDropdownColors(dropdown, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not dropdown or not dropdown.guiBackdrop then return end
    local btnBgR = math.min(bgr + 0.07, 1)
    local btnBgG = math.min(bgg + 0.07, 1)
    local btnBgB = math.min(bgb + 0.07, 1)
    dropdown.guiBackdrop:SetBackdropColor(btnBgR, btnBgG, btnBgB, 1)
...
local function RefreshInstanceFramesColors()
    local PVEFrame = _G.PVEFrame
    if not PVEFrame or not PVEFrame.guiSkinned then return end

    local sr, sg, sb, sa, bgr, bgg, bgb, bga = GetColors()

    -- Update PVEFrame backdrop
    if PVEFrame.guiBackdrop then
        PVEFrame.guiBackdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        PVEFrame.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
    ...
    local RaidFinderQueueFrame = _G.RaidFinderQueueFrame
    if RaidFinderQueueFrame and RaidFinderQueueFrame.guiSkinned then
    ...
    local LFGListFrame = _G.LFGListFrame
    if LFGListFrame and LFGListFrame.guiSkinned then
        if LFGListFrame.CategorySelection then
        ...
        if LFGListFrame.SearchPanel then
            ...
            if LFGListFrame.SearchPanel.SearchBox and LFGListFrame.SearchPanel.SearchBox.guiBackdrop then
                LFGListFrame.SearchPanel.SearchBox.guiBackdrop:SetBackdropColor(bgr, bgg, bgb, bga)
                LFGListFrame.SearchPanel.SearchBox.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
            end
        end
        if LFGListFrame.ApplicationViewer then
    ...
    local ChallengesFrame = _G.ChallengesFrame
    if ChallengesFrame and ChallengesFrame.guiSkinned then
        if ChallengesFrame.DungeonIcons then
            for _, icon in pairs(ChallengesFrame.DungeonIcons) do
    ...
    local PVPQueueFrame = _G.PVPQueueFrame
    if PVPQueueFrame and PVPQueueFrame.guiSkinned then
        ...
        local TrainingGroundsFrame = _G.TrainingGroundsFrame
        if TrainingGroundsFrame and TrainingGroundsFrame.guiSkinned then
        ...
        local PlunderstormFrame = _G.PlunderstormFrame
        if PlunderstormFrame and PlunderstormFrame.guiSkinned then
...
_G.GravityUI_RefreshInstanceFramesColors = RefreshInstanceFramesColors
