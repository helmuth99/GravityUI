...
-- gui skinning for ReadyCheckFrame
---------------------------------------------------------------------------
...
_G.GravityUI_ResetReadyCheckPosition = ResetReadyCheckPosition
...
_G.GravityUI_ToggleReadyCheckMover = ToggleMover

---------------------------------------------------------------------------
-- HELPER FUNCTIONS
---------------------------------------------------------------------------

-- Get gui skin colors with fallback
...
-- Create gui-styled backdrop on a frame
local function CreateguiBackdrop(frame)
...
-- Style a button with gui look
local function SkinButton(button, sr, sg, sb, bgr, bgg, bgb, bga)
    if not button or button.guiSkinned then return end
    ...
    -- Create backdrop
    local backdrop = CreateguiBackdrop(button)
    ...
    button.guiNormalBg = { btnBgr, btnBgg, btnBgb, bga }
    button.guiHoverBg = { math.min(btnBgr + 0.1, 1), math.min(btnBgg + 0.1, 1), math.min(btnBgb + 0.1, 1), bga }
    button.guiBorderColor = { sr, sg, sb, 1 }

    -- Hover effects
    button:HookScript("OnEnter", function(self)
        if self.guiBackdrop and self.guiHoverBg then
            self.guiBackdrop:SetBackdropColor(unpack(self.guiHoverBg))
        end
    end)
    button:HookScript("OnLeave", function(self)
        if self.guiBackdrop and self.guiNormalBg then
            self.guiBackdrop:SetBackdropColor(unpack(self.guiNormalBg))
        end
    end)
...
local function SkinReadyCheckFrame()
    ...
    -- Create gui backdrop on ListenerFrame (where the content is)
    local targetFrame = listenerFrame or frame
    local backdrop = CreateguiBackdrop(targetFrame)
    ...
    if not frame.guiTitle then
        ...
        frame.guiTitle:SetFont(STANDARD_TEXT_FONT, 13, FONT_FLAGS)
    end
    frame.guiTitle:SetText("Ready Check")
    frame.guiTitle:SetTextColor(sr, sg, sb, 1)  -- Use skin color for title
...
_G.GravityUI_RefreshReadyCheckColors = RefreshReadyCheckColors
