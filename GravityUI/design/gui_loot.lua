...
local function GetThemeColors()
    ...
    if gui and gui.GetSkinColor and gui.GetSkinBgColor then
        local sr, sg, sb, sa = gui:GetSkinColor()
...
local function GetGeneralFont()
    local db = guiCore.db and guiCore.db.profile and guiCore.db.profile.general
    return (db and db.font) or "Gravity"
end

local function GetDB()
    return guiCore.db and guiCore.db.profile or {}
end
...
local function CreateLootWindow()
    ...
    local frame = CreateFrame("Frame", "gui_LootFrame", UIParent, "BackdropTemplate")
...
local function CreateRollFrame(index)
    ...
    local frame = CreateFrame("Frame", "gui_LootRollFrame"..index, UIParent, "BackdropTemplate")
    ...
    frame.timer:SetStatusBarTexture(LSM:Fetch("statusbar", "Gravity") or "Interface\\TargetingFrame\\UI-StatusBar")
...
local function CreateRollAnchor()
    local anchor = CreateFrame("Frame", "gui_LootRollAnchor", UIParent, "BackdropTemplate")
...
local function SkinLootHistoryElement(button)
    if button.guiSkinned then return end
    ...
    if item then
        ...
        if icon then
            ...
            if not item.guiBorder then
                ...
                item.guiBorder:SetPoint("BOTTOMRIGHT", icon, "BOTTOMRIGHT", 1, -1)
                item.guiBorder:SetBackdrop({ edgeFile = "Interface\\Buttons\\WHITE8x8", edgeSize = 1 })
                item.guiBorder:SetBackdropBorderColor(0.6, 0.6, 0.6, 1)
            end
            ...
            if item.IconBorder and not item._guiBorderHooked then
                item._guiBorderHooked = true
                hooksecurefunc(item.IconBorder, "SetVertexColor", function(self, r, g, b)
                    if item.guiBorder then
                        item.guiBorder:SetBackdropBorderColor(r, g, b, 1)
                    end
                end)
...
local function SkinGroupLootHistoryFrame()
    ...
    if not HistoryFrame.guiBackdrop then
        ...
        HistoryFrame.guiBackdrop:SetFrameLevel(HistoryFrame:GetFrameLevel())
        HistoryFrame.guiBackdrop:SetBackdrop({
            bgFile = "Interface\\Buttons\\WHITE8x8",
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = 1,
        })
    end
    HistoryFrame.guiBackdrop:SetBackdropColor(unpack(bgColor))
    HistoryFrame.guiBackdrop:SetBackdropBorderColor(unpack(borderColor))
    ...
    if Timer then
        ...
        if Timer.Fill then
            Timer.Fill:SetTexture(LSM:Fetch("statusbar", "Gravity") or "Interface\\TargetingFrame\\UI-StatusBar")
            Timer.Fill:SetVertexColor(borderColor[1], borderColor[2], borderColor[3], 1)
        end

        -- Add timer background
        if not Timer.guiBg then
            Timer.guiBg = Timer:CreateTexture(nil, "BACKGROUND")
            Timer.guiBg:SetAllPoints()
            Timer.guiBg:SetColorTexture(0, 0, 0, 0.5)
        end
    end
    ...
    if ResizeButton then
        ...
        if not ResizeButton.guiBackdrop then
            ResizeButton.guiBackdrop = CreateFrame("Frame", nil, ResizeButton, "BackdropTemplate")
            ResizeButton.guiBackdrop:SetAllPoints()
            ResizeButton.guiBackdrop:SetBackdrop({
                bgFile = "Interface\\Buttons\\WHITE8x8",
                edgeFile = "Interface\\Buttons\\WHITE8x8",
                edgeSize = 1,
            })
            ResizeButton.guiBackdrop:SetBackdropColor(bgColor[1], bgColor[2], bgColor[3], 0.8)
            ResizeButton.guiBackdrop:SetBackdropBorderColor(unpack(borderColor))

            -- Add resize text
            ResizeButton.guiText = ResizeButton:CreateFontString(nil, "OVERLAY")
            ResizeButton.guiText:SetFont(LSM:Fetch("font", GetGeneralFont()), 12, "OUTLINE")
            ResizeButton.guiText:SetPoint("CENTER")
            ResizeButton.guiText:SetText("v v v")
            ResizeButton.guiText:SetTextColor(unpack(textColor))
        end
    end
...
function Loot:ApplyLootHistoryTheme()
    ...
    if not enabled then
        if HistoryFrame.guiBackdrop then
            HistoryFrame.guiBackdrop:Hide()
        end
        if HistoryFrame.NineSlice then
        ...
        if HistoryFrame.Timer then
            ...
            if HistoryFrame.Timer.guiBg then HistoryFrame.Timer.guiBg:Hide() end
        end
        if HistoryFrame.ResizeButton and HistoryFrame.ResizeButton.guiBackdrop then
            HistoryFrame.ResizeButton.guiBackdrop:Hide()
            if HistoryFrame.ResizeButton.NineSlice then
                HistoryFrame.ResizeButton.NineSlice:SetAlpha(1)
            end
            if HistoryFrame.ResizeButton.guiText then
                HistoryFrame.ResizeButton.guiText:Hide()
            end
        end
        return
    end
    ...
    if HistoryFrame.Timer then
        ...
        if HistoryFrame.Timer.guiBg then HistoryFrame.Timer.guiBg:Show() end
    ...
    if HistoryFrame.ResizeButton and HistoryFrame.ResizeButton.guiBackdrop then
        HistoryFrame.ResizeButton.guiBackdrop:Show()
        if HistoryFrame.ResizeButton.NineSlice then
            HistoryFrame.ResizeButton.NineSlice:SetAlpha(0)
        end
        HistoryFrame.ResizeButton.guiBackdrop:SetBackdropColor(bgColor[1], bgColor[2], bgColor[3], 0.8)
        HistoryFrame.ResizeButton.guiBackdrop:SetBackdropBorderColor(unpack(borderColor))
        if HistoryFrame.ResizeButton.guiText then
            HistoryFrame.ResizeButton.guiText:Show()
            HistoryFrame.ResizeButton.guiText:SetTextColor(unpack(textColor))
        end
    end
end
...
local function DisableBlizzardLoot()
    ...
    if db.lootRoll and db.lootRoll.enabled then
        ...
        if GroupLootContainer then
            ...
            if not GroupLootContainer._guiHooked then
                hooksecurefunc(GroupLootContainer, "Show", function(self)
                    self:Hide()
                end)
                GroupLootContainer._guiHooked = true
            end
        end
        ...
        for i = 1, numRollFrames do
            ...
            if frame then
                ...
                frame:Hide()
                if not frame._guiHooked then
                    hooksecurefunc(frame, "Show", function(self)
                        self:Hide()
                    end)
                    frame._guiHooked = true
                end
            end
        end
    end
...
_G.GravityUI_RefreshLootColors = function()
    if guiCore and guiCore.Loot then
        guiCore.Loot:RefreshColors()
    end
end
