...
-- with a clean gui-styled bar. Used for: Atramedes sound, Cho'gall
...
local function GetDB()
    ...
    return guiCore and guiCore.db and guiCore.db.profile or {}
end
...
local function CreateguiAltPowerBar()
    -- Get skin colors
    ...
    local bar = CreateFrame("StatusBar", "GravityUI_AltPowerBar", UIParent)
    ...
    bar.guiSkinColor = { sr, sg, sb, sa }
    bar.guiBgColor = { bgr, bgg, bgb, bga }
    bar.guiSkinned = true
...
local function RefreshPowerBarAltColors()
    if not guiAltPowerBar then return end
    ...
    guiAltPowerBar:SetStatusBarColor(sr, sg, sb)
    guiAltPowerBar.backdrop:SetBackdropColor(bgr, bgg, bgb, bga)
    guiAltPowerBar.backdrop:SetBackdropBorderColor(sr, sg, sb, sa)

    guiAltPowerBar.guiSkinColor = { sr, sg, sb, sa }
    guiAltPowerBar.guiBgColor = { bgr, bgg, bgb, bga }
...
_G.GravityUI_RefreshPowerBarAltColors = RefreshPowerBarAltColors
...
local function CreateMover()
    ...
    powerBarMover:SetPoint("CENTER", guiAltPowerBar, "CENTER")
    ...
    powerBarMover:SetScript("OnDragStart", function(self)
        guiAltPowerBar:StartMoving()
    end)

    powerBarMover:SetScript("OnDragStop", function(self)
        guiAltPowerBar:StopMovingOrSizing()
        -- Save position
        local point, _, relPoint, x, y = guiAltPowerBar:GetPoint()
        SaveBarPosition(point, relPoint, x, y)
        -- Re-anchor mover to bar
        self:ClearAllPoints()
        self:SetPoint("CENTER", guiAltPowerBar, "CENTER")
    end)
end
...
local function ShowMover()
    ...
    if powerBarMover then
        ...
        if guiAltPowerBar then
            guiAltPowerBar:Show()
            -- Show placeholder if no active power bar
            if not guiAltPowerBar.powerName then
                guiAltPowerBar.text:SetText("Encounter Power Bar")
                guiAltPowerBar:SetMinMaxValues(0, 100)
                guiAltPowerBar:SetValue(50)
            end
        end
    end
end
...
_G.GravityUI_TogglePowerBarAltMover = ToggleMover
...
local function Initialize()
    ...
    -- Create our bar
    guiAltPowerBar = CreateguiAltPowerBar()
...
frame:SetScript("OnEvent", function(self, event)
    ...
    -- Delay slightly to ensure gui is loaded
