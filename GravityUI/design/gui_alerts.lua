--- GravityUI Alert & Toast Skinning
--- Skins Blizzard alert frames with GUI styling and adds movers
...
-- Text color
local GUI_TEXT_COLOR = { 0.953, 0.957, 0.965, 1 }
...
--- Get theme colors from GUI skinning system
...
--- Create GUI-styled backdrop for alert frames
local function CreateAlertBackdrop(frame, xOffset1, yOffset1, xOffset2, yOffset2)
    if frame.guiBackdrop then return frame.guiBackdrop end
...
local function CreateIconBorder(icon, parent, qualityColor)
    ...
    if icon.guiBorder then
        if qualityColor then
            icon.guiBorder:SetBackdropBorderColor(qualityColor.r or qualityColor[1], qualityColor.g or qualityColor[2], qualityColor.b or qualityColor[3], 1)
        else
            icon.guiBorder:SetBackdropBorderColor(sr, sg, sb, sa)
        end
        return icon.guiBorder
    end
...
local function SkinAchievementAlert(frame)
    ...
    if frame.Unlocked then
        frame.Unlocked:SetTextColor(unpack(GUI_TEXT_COLOR))
    end
    if frame.Name then
...
local function SkinCriteriaAlert(frame)
    ...
    if frame.Unlocked then frame.Unlocked:SetTextColor(unpack(GUI_TEXT_COLOR)) end
...
local function SkinLootWonAlert(frame)
    ...
    if not frame.guiBackdrop and lootItem.Icon.guiBorder then
        ...
        local backdrop = CreateFrame("Frame", nil, frame, "BackdropTemplate")
        backdrop:SetFrameLevel(frame:GetFrameLevel())
        backdrop:SetPoint("TOPLEFT", lootItem.Icon.guiBorder, "TOPLEFT", -4, 4)
        backdrop:SetPoint("BOTTOMRIGHT", lootItem.Icon.guiBorder, "BOTTOMRIGHT", 180, -4)
        backdrop:SetBackdrop({
            bgFile = "Interface\\Buttons\\WHITE8x8",
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = 1,
...
local function SkinLootUpgradeAlert(frame)
    ...
    -- Create backdrop
    if not frame.guiBackdrop and frame.Icon.guiBorder then
        local sr, sg, sb, sa, bgr, bgg, bgb, bga = GetThemeColors()

        local backdrop = CreateFrame("Frame", nil, frame, "BackdropTemplate")
        backdrop:SetFrameLevel(frame:GetFrameLevel())
        backdrop:SetPoint("TOPLEFT", frame.Icon.guiBorder, "TOPLEFT", -8, 8)
        backdrop:SetPoint("BOTTOMRIGHT", frame.Icon.guiBorder, "BOTTOMRIGHT", 180, -8)
        backdrop:SetBackdrop({
            bgFile = "Interface\\Buttons\\WHITE8x8",
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = 1,
...
local function SkinHonorAwardedAlert(frame)
    ...
    if not frame.guiBackdrop and frame.Icon.guiBorder then
        ...
        local backdrop = CreateFrame("Frame", nil, frame, "BackdropTemplate")
        backdrop:SetFrameLevel(frame:GetFrameLevel())
        backdrop:SetPoint("TOPLEFT", frame.Icon.guiBorder, "TOPLEFT", -4, 4)
        backdrop:SetPoint("BOTTOMRIGHT", frame.Icon.guiBorder, "BOTTOMRIGHT", 180, -4)
        backdrop:SetBackdrop({
            bgFile = "Interface\\Buttons\\WHITE8x8",
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = 1,
...
local function SkinNewRecipeLearnedAlert(frame)
    ...
    if frame.Icon then
        ...
        frame.Icon:SetPoint("LEFT", frame.guiBackdrop, 9, 0)
...
local function SkinScenarioAlert(frame)
    ...
    if frame.dungeonTexture then
        ...
        frame.dungeonTexture:ClearAllPoints()
        frame.dungeonTexture:SetPoint("LEFT", frame.guiBackdrop, 9, 0)
        frame.dungeonTexture:SetDrawLayer("OVERLAY")
...
local function SkinMiscAlert(frame)
    ...
    if frame.guiSkinned then return end
    ...
    if frame.Icon then
        ...
        local border = frame.Icon.guiBorder
        frame.Icon:SetParent(border)
...
local function SkinBonusRollFrames()
    ...
    if moneyFrame and not moneyFrame.guiSkinned then
        ...
        backdrop:SetPoint("TOPLEFT", moneyFrame.Icon.guiBorder, "TOPLEFT", -4, 4)
        backdrop:SetPoint("BOTTOMRIGHT", moneyFrame.Icon.guiBorder, "BOTTOMRIGHT", 180, -4)
        backdrop:SetBackdrop({
            bgFile = "Interface\\Buttons\\WHITE8x8",
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = 1,
        ...
        backdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        backdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        moneyFrame.guiBackdrop = backdrop
        moneyFrame.guiSkinned = true
    end

    -- BonusRollLootWonFrame
    local lootFrame = BonusRollLootWonFrame
    if lootFrame and not lootFrame.guiSkinned then
        ...
        backdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        backdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        lootFrame.guiBackdrop = backdrop
        lootFrame.guiSkinned = true
    end
end
...
local POSITION, ANCHOR_POINT, Y_OFFSET = "BOTTOM", "TOP", 5
...
local function CreateAlertMover()
    ...
    if not alertHolder then
        alertHolder = CreateFrame("Frame", "GUI_AlertFrameHolder", UIParent)
        ...
        alertMover = CreateFrame("Frame", "GUI_AlertFrameMover", alertHolder, "BackdropTemplate")
...
local function CreateEventToastMover()
    ...
    if not toastHolder then
        toastHolder = CreateFrame("Frame", "GUI_EventToastHolder", UIParent)
        ...
        -- Create mover overlay
        toastMover = CreateFrame("Frame", "GUI_EventToastMover", toastHolder, "BackdropTemplate")
...
local function RefreshAlertColors()
    ...
    for _, system in ipairs(alertSystems) do
        if system and system.alertFramePool then
            for frame in system.alertFramePool:EnumerateActive() do
                ...
                -- Update icon borders
                if frame.Icon and frame.Icon.guiBorder then
                    frame.Icon.guiBorder:SetBackdropBorderColor(sr, sg, sb, sa)
                end
            end
        end
    end
    ...
    if BonusRollMoneyWonFrame and BonusRollMoneyWonFrame.guiBackdrop then
        BonusRollMoneyWonFrame.guiBackdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        BonusRollMoneyWonFrame.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
    if BonusRollLootWonFrame and BonusRollLootWonFrame.guiBackdrop then
        BonusRollLootWonFrame.guiBackdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        BonusRollLootWonFrame.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
end

-- Expose refresh function globally
_G.GravityUI_RefreshAlertColors = RefreshAlertColors
