---------------------------------------------------------------------------
-- GravityUI Mythic+ Timer Skinning
-- Applies gui skin colors to the custom M+ timer frame
-- Frame created in utils/gui_mplus_timer.lua
---------------------------------------------------------------------------
local ADDON_NAME, ns = ...
...
local function ApplyBackdrop(frame, sr, sg, sb, sa, bgr, bgg, bgb, bga, showBorder)
    ...
    if not frame.guiBackdrop then
        ...
        frame.guiBackdrop:SetFrameLevel(math.max(1, frame:GetFrameLevel() - 1))
    ...
    frame.guiBackdrop:SetBackdropBorderColor(sr, sg, sb, borderAlpha)
end
...
local function ApplyMPlusTimerSkin()
    ...
    MPlusTimer.frames.root.guiSkinned = true
    MPlusTimer.frames.root.guiColors = colors
end
...
local function RefreshMPlusTimerColors()
    ...
    if not MPlusTimer.frames.root.guiSkinned then return end
...
---------------------------------------------------------------------------
_G.GravityUI_ApplyMPlusTimerSkin = ApplyMPlusTimerSkin
_G.GravityUI_RefreshMPlusTimerColors = RefreshMPlusTimerColors
