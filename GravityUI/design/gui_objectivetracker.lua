...
-- Applies gui color scheme with dynamic content-height backdrop
---------------------------------------------------------------------------
...
-- Style completion checkmark with gui color
...
-- Sync gui max height with Blizzard's editModeHeight so truncation matches backdrop
...
local function ApplyguiBackdrop(trackerFrame, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    -- Create gui backdrop (anchors will be set by UpdateBackdropAnchors)
...
local function GetFontPath()
    local gui = _G.GravityUI
    return gui and gui.GetGlobalFont and gui:GetGlobalFont() or STANDARD_TEXT_FONT
end
...
local function SkinObjectiveTracker()
    ...
    -- Apply gui backdrop with our colors/opacity
...
_G.GravityUI_RefreshObjectiveTracker = RefreshObjectiveTracker
_G.GravityUI_RefreshObjectiveTrackerColors = RefreshObjectiveTracker
