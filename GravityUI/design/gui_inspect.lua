...
guiCore.InspectSkinning = InspectSkinning

-- Configuration constants (match character.lua for consistency)
local CONFIG = {
...
local function GetSkinColors()
    ...
    local sr, sg, sb, sa = 0, 0.74901960784314, 1      -- Fallback deepbluesky
...
local function IsSkinningEnabled()
    local coreRef = _G.GravityUI and _G.GravityUI.guiCore
...
local function CreateOrUpdateBackground()
    ...
    if not customBg then
        customBg = CreateFrame("Frame", "gui_InspectFrameBg_Skin", InspectFrame, "BackdropTemplate")
...
---------------------------------------------------------------------------
-- Main skinning setup
-- Note: OnShow hook is handled by gui_character.lua which calls SetExtended()
...
local function SetupInspectFrameSkinning()
    ...
    -- Create initial background (will be positioned by gui_character.lua via SetExtended)
    CreateOrUpdateBackground()
...
_G.GravityUI_RefreshInspectColors = RefreshInspectFrameColors
