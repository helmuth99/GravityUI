---------------------------------------------------------------------------
-- GravityUI - Character Panel Module
-- Custom character panel styling with equipment overlays and stats panel
-- Ported from GravityUI (1:1 feature parity)
---------------------------------------------------------------------------
local ADDON_NAME, ns = ...
...
---------------------------------------------------------------------------
-- Get global font from gui settings
---------------------------------------------------------------------------
local function GetGlobalFont()
    local guiCore = ns.GetDB()
    if guiCore and guiCore.db and guiCore.db.profile then
        local fontName = guiCore.db.profile.general and guiCore.db.profile.general.font
        if fontName then
...
local function UpdateSlotOverlay(overlay, unit)
    ...
    if overlay.backdropBar then
        if settings.showBackdrops ~= false and itemLink then
            local r, g, b = 0, 0, 0
            local alpha = 0.4
...
local function HideBlizzardDecorations()
    ...
    for _, frameName in ipairs(slotFrames) do
        ...
        if frame then
            ...
            if not frame._guiHideHooked then
                hooksecurefunc(frame, "Show", function(self) self:Hide() end)
                frame._guiHideHooked = true
            end
        end
    end
    ...
    local function SkinEquipmentSlot(slot)
        ...
        -- Block Blizzard's IconBorder (we use custom _guiBorderFrame instead)
        ...
        if not slot._guiBorderFrame then
            ...
            slot._guiBorderFrame:SetBackdrop({
                edgeFile = "Interface\\Buttons\\WHITE8X8",
                edgeSize = 1,
            })
        end
    ...
    if firstSlot and not firstSlot._guiEquipHooked then
        ...
        firstSlot._guiEguipHooked = true
    end
...
local function CreateCustomBackground()
    ...
    if skinningAPI and skinningAPI.IsEnabled and skinningAPI.IsEnabled() then
        ...
        -- Use configurable panel color if set, otherwise fallback to skin colors
        ...
        if not customBg then
            customBg = CreateFrame("Frame", "gui_CharacterFrameBg_CharPane", CharacterFrame, "BackdropTemplate")
...
local function SetupTitleArea()
    ...
    if not CharacterFrame._guiILvlDisplay then
...
local function AcquireStatRow(parent)
    ...
    if not row then
        ...
        row:SetSize(200, 14) -- Default size, updated later
...
local function AcquireStatBar(parent)
    local row = table.remove(statBarPool)
    if not row then
        row = CreateFrame("Frame", nil, parent)
        row:SetSize(200, 16)
...
_G.GravityUI_RefreshCharacterPanelFonts = RefreshCharacterPanelFonts
...
local function CreateStatRow(parent, yOffset)
    ...
    local row = AcquireStatRow(parent)
...
local function CreateStatBar(parent, yOffset, color)
    ...
    local row = AcquireStatBar(parent)
    row:SetSize(parent:GetWidth() - 10, rowHeight)
    row:SetPoint("TOPLEFT", 5, yOffset)
    ...
    -- Bar background (already created in Acquire)
    row.bar.bg:SetAllPoints()
...
local function UpdateStatsPanel(panel, unit)
    ...
    -- ATTRIBUTES
    -- In M+ (Challenge Mode) since 12.0.5, UnitStat and all primary-stat APIs
    -- return tainted "secret numbers" — they cannot be converted, compared, or
    -- formatted without crashing. Skip them entirely and show a notice instead.
    local inChallengeMode = C_ChallengeMode and C_ChallengeMode.IsChallengeModeActive and
    ...
    -- In M+: force percent-only for secondary stats.
    -- GetCritChance/GetHaste/etc. return normal (untainted) floats even in Challenge Mode.
    -- Rating values come from GetCombatRating which CAN be secret — avoid in M+.
    local statFormat = inChallengeMode and "percent" or (settings.secondaryStatFormat or "percent")

    for _, stat in ipairs(SECONDARY_STATS_DEF) do
        local percentValue = SafeGetStat(stat.percentFunc) or 0
        local ratingValue = inChallengeMode and 0 or (SafeGetStat(stat.ratingFunc) or 0)
        ...
        -- in M+ (Challenge Mode) since 12.0.5. Wrap each block in a pcall so a crash here
        -- never aborts the loop and the remaining stats still render.
...
local function UpdateILvlDisplay()
    if not CharacterFrame or not CharacterFrame._guiILvlDisplay then return end

    local settings = GetSettings()
    if not settings.enabled then return end

    local displayFrame = CharacterFrame._guiILvlDisplay
...
-- Refresh stats panel when entering/leaving a Mythic+ key so the
-- panel switches between normal view and M+ percent-only view.
do
    local cmEventFrame = CreateFrame("Frame")
    cmEventFrame:RegisterEvent("CHALLENGE_MODE_START")
    cmEventFrame:RegisterEvent("CHALLENGE_MODE_COMPLETED")
    cmEventFrame:RegisterEvent("CHALLENGE_MODE_RESET")
    cmEventFrame:SetScript("OnEvent", function()
        ScheduleStatUpdate()
    end)
end
...
local function HookCharacterFrame()
    ...
    if PaperDollSidebarTab3 and not PaperDollSidebarTab3._guiHooked then
        PaperDollSidebarTab3:HookScript("OnClick", function()
            ...
            if pane then
                ...
                if not pane._guiOriginalParent then
                    pane._guiOriginalParent = pane:GetParent()
                end
        ...
        PaperDollSidebarTab3._guiHooked = true
    end

    -- Titles tab (Tab2): Reparent to floating popup
    if PaperDollSidebarTab2 and not PaperDollSidebarTab2._guiHooked then
        ...
        PaperDollSidebarTab2._guiHooked = true
    end
...
eventFrame:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" then
        -- Note: Blizzard_InspectUI is now hooked by gui_inspect.lua
...
---------------------------------------------------------------------------
-- Module API (Public interface for GravityUI)
---------------------------------------------------------------------------
Character.RefreshCharacterPane = function()
    ScheduleUpdate()
end

Character.RefreshAllFonts = RefreshCharacterPanelFonts

Character.RefreshBackground = function()
    -- Only refresh if layout is already applied (meaning frame is shown)
    if CharacterFrame and CharacterFrame:IsShown() then
        CreateCustomBackground()
    end
end

Character.GetSettings = GetSettings
