...
local BLOODLUST_SPELLS = {
    ...
    [102364] = true, -- Drums (Generic, check specific IDs if needed, usually mapped to a spell)
    -- Drums of Deathly Ferocity / Mountain etc often cast a spell like 309658 or similar.
...
local BL_MEDIA = {
    ["goku"] = { 
        texture = "Interface/AddOns/GravityUI/assets/media/BL/goku.tga",
        type = "flipbook",
        cols = 3, rows = 6, frames = 18, speed = 0.08, loop = true
    },
    ["KP_Demon"] = { 
        texture = "Interface/AddOns/GravityUI/assets/media/BL/KP_Demon.tga",
        type = "flipbook",
        cols = 8, rows = 8, frames = 64, speed = 0.06, loop = true
    },    
    ["pedro"] = { 
        texture = "Interface/AddOns/GravityUI/assets/media/BL/pedro.tga", -- Pointing to pedro.tga as requested
        type = "flipbook",
        cols = 4, rows = 8, frames = 32, speed = 0.05, loop = true
    },
}

local BL_SOUNDS = {
    ["Kamehameha"] = "Interface/AddOns/GravityUI/assets/media/BL/Kamehameha.ogg",
    ["pedro"] = "Interface/AddOns/GravityUI/assets/media/BL/pedro.ogg",
    ["KP_Demon"] = "Interface/AddOns/GravityUI/assets/media/BL/KP_Demon.ogg",
}
...
local function GetBloodlustSettings()
    local db = ns.GetDB()
    return db and db.screenindicators and db.screenindicators.bloodlust
end

local function CreateBloodlustFrame()
    if BloodlustState.frame then return end
    
    local f = CreateFrame("Frame", "GravityUI_Bloodlust", UIParent)
    f:SetSize(256, 256)
    f:SetPoint("CENTER", 0, 0)
    f:SetFrameStrata("DIALOG") -- High priority
    f:Hide()
    
    local t = f:CreateTexture(nil, "ARTWORK")
    t:SetAllPoints()
    BloodlustState.texture = t
    
    local txt = f:CreateFontString(nil, "OVERLAY", "GameFontNormalHuge")
    txt:SetPoint("BOTTOM", f, "TOP", 0, 10)
    BloodlustState.text = txt
    
    -- Animation Groups (Created on demand or pre-created)
    -- We attach them to the frame for easy access
    
    -- 1. Pop (Default / Entrance)
    local agPop = f:CreateAnimationGroup()
    local p1 = agPop:CreateAnimation("Scale")
    p1:SetOrder(1); p1:SetDuration(0.2); p1:SetScaleFrom(0.1, 0.1); p1:SetScaleTo(1.2, 1.2); p1:SetSmoothing("OUT")
    local p2 = agPop:CreateAnimation("Scale")
    p2:SetOrder(2); p2:SetDuration(0.1); p2:SetScaleFrom(1.2, 1.2); p2:SetScaleTo(1.0, 1.0); p2:SetSmoothing("IN")
    -- We remove the built-in Hold/Fade from agPop so we can control duration dynamically via Lua
    BloodlustState.agPop = agPop

    -- 2. Shake (Goku / Power Up)
    local agShake = f:CreateAnimationGroup()
    -- Initial Pop
    local s1 = agShake:CreateAnimation("Scale")
    s1:SetOrder(1); s1:SetDuration(0.2); s1:SetScaleFrom(0.1, 0.1); s1:SetScaleTo(1.0, 1.0); s1:SetSmoothing("OUT")
    -- Violent Shake (Translation loop)
    local shakeDuration = 2.5
    local shakes = 10
    for i=1, shakes do
        local t = agShake:CreateAnimation("Translation")
        t:SetOrder(1) -- Concurrent with hold? No, sequential after pop.
        t:SetStartDelay(0.2 + ((i-1)*(shakeDuration/shakes)))
        t:SetDuration(shakeDuration/shakes)
        -- Random wiggle
        local x = (i % 2 == 0) and 10 or -10
        local y = (i % 3 == 0) and 10 or -10
        t:SetOffset(x, y)
        t:SetSmoothing("NONE")
    end
    -- Fade handled manually now for consistency
    BloodlustState.agShake = agShake

    -- 3. Spin (Pedro)
    local agSpin = f:CreateAnimationGroup()
    -- Pop
    local sp1 = agSpin:CreateAnimation("Scale")
    sp1:SetOrder(1); sp1:SetDuration(0.2); sp1:SetScaleFrom(0.1, 0.1); sp1:SetScaleTo(1.0, 1.0)
    -- Rotate
    local rot = agSpin:CreateAnimation("Rotation")
    rot:SetOrder(1); rot:SetDuration(3.0); rot:SetDegrees(360 * 2) -- 2 spins
    BloodlustState.agSpin = agSpin
    
    -- Fade Out Animation (Generic)
    local agFade = f:CreateAnimationGroup()
    local f1 = agFade:CreateAnimation("Alpha")
    f1:SetDuration(0.5); f1:SetFromAlpha(1); f1:SetToAlpha(0)
    agFade:SetScript("OnFinished", function() f:Hide() end)
    BloodlustState.agFade = agFade
    
    BloodlustState.frame = f
    BloodlustState.texture = t 
    BloodlustState.text = txt
    
    -- Mover Integration
    local function BloodlustMoverToggle(frame, visible)
        if visible then
            ...
            if not frame:GetScript("OnDragStart") then
                ...
                frame:SetScript("OnDragStop", function(self)
                    self:StopMovingOrSizing()
                    -- Save Position
                    local s = GetBloodlustSettings()
                    if s then
                        local point, _, relativePoint, xOfs, yOfs = self:GetPoint()
                        s.pos = { point = point, relativePoint = relativePoint, x = xOfs, y = yOfs }
                    end
                end)
            end
            BloodlustState.text:SetText("MOVER (Drag Me)")
            BloodlustState.text:Show()
            BloodlustState.texture:SetAlpha(0.5) -- Dim art slightly
            
            -- Add a visible background for easier grabbing if art is weird shape
            if not frame.bg then
                frame.bg = frame:CreateTexture(nil, "BACKGROUND")
                frame.bg:SetAllPoints()
                frame.bg:SetColorTexture(0, 0.8, 0, 0.3)
            end
            frame.bg:Show()
        else
            frame:EnableMouse(false)
            frame:SetMovable(false)
            BloodlustState.text:Hide()
            BloodlustState.texture:SetAlpha(1)
    ...
    if s and s.pos then
        f:ClearAllPoints()
        f:SetPoint(s.pos.point, UIParent, s.pos.relativePoint, s.pos.x, s.pos.y)
    end

    if ns.Movers and ns.Movers.Register then
        ns.Movers:Register("GravityUI_Bloodlust", f, BloodlustMoverToggle, "Bloodlust Alert")
    end
end

function Screen.StopBloodlust()
    if BloodlustState.frame then
        BloodlustState.frame:Hide()
        BloodlustState.frame:SetScript("OnUpdate", nil)
        if BloodlustState.agPop then BloodlustState.agPop:Stop() end
        if BloodlustState.agShake then BloodlustState.agShake:Stop() end
        if BloodlustState.agSpin then BloodlustState.agSpin:Stop() end
        if BloodlustState.agFade then BloodlustState.agFade:Stop() end
    end
    
    if BloodlustState.soundHandle then
        StopSound(BloodlustState.soundHandle)
        BloodlustState.soundHandle = nil
    end
    
    if BloodlustState.timer then
        BloodlustState.timer:Cancel()
        BloodlustState.timer = nil
    end
end

function Screen.TriggerBloodlust(sourceName)
    local s = GetBloodlustSettings()
    if not s or not s.enabled then return end
    
    CreateBloodlustFrame()
    Screen.StopBloodlust() -- Reset any running state first
    
    local f = BloodlustState.frame
    
    -- 1. Visual Setup
    local style = s.texture or "goku"
    local media = BL_MEDIA[style] or BL_MEDIA["goku"]
    
    if media then
        BloodlustState.texture:SetTexture(media.texture)
    else
        BloodlustState.texture:SetTexture("Interface/AddOns/GravityUI/assets/media/BL/goku.tga")
    end
    
    -- Size
    local size = s.size or 256
    f:SetSize(size, size)
    
    -- Removed "Used by" text as requested
    BloodlustState.text:Hide()
    ...
    -- 2. Animation Logic
    BloodlustState.texture:SetTexCoord(0, 1, 0, 1) -- Reset coords
    
    -- ALL styles now run for 40 seconds (or until stopped)
    -- We play the entrance animation, then wait, then fade out.
    
    local entranceAnim = nil
    -- Always use Pop for flipbooks to match Pedro behavior as requested
    if media.type == "flipbook" then
         entranceAnim = BloodlustState.agPop
    else
         -- Fallback for static if any end up here
         if style == "goku" then
             entranceAnim = BloodlustState.agShake
         else
             entranceAnim = BloodlustState.agPop
         end
    end
    
    if entranceAnim then entranceAnim:Play() end
    
    -- Loop Logic
    local totalDuration = 40 
    ...
    if media.type == "flipbook" then
        ...
        f:SetScript("OnUpdate", function(self, elapsed)
            ...
            if (now - startTime) > totalDuration then
                ...
                if BloodlustState.agFade then BloodlustState.agFade:Play() end
                return
            end
            
            timeSinceLastFrame = timeSinceLastFrame + elapsed
            if timeSinceLastFrame >= media.speed then
                local framesToAdvance = math.floor(timeSinceLastFrame / media.speed)
                currentFrame = (currentFrame + framesToAdvance) % media.frames
                timeSinceLastFrame = timeSinceLastFrame % media.speed 

                local col = currentFrame % media.cols
                local row = math.floor(currentFrame / media.cols)
                
                local left = col / media.cols
                local right = (col + 1) / media.cols
                local top = row / media.rows
                local bottom = (row + 1) / media.rows
                
                BloodlustState.texture:SetTexCoord(left, right, top, bottom) 
            end
        end)
    else
        -- Static image, just wait
        BloodlustState.timer = C_Timer.NewTimer(totalDuration, function()
             if BloodlustState.agFade then BloodlustState.agFade:Play() end
        end)
    end
    
    -- 3. Audio
    local soundFile = BL_SOUNDS[s.sound]
    if not soundFile and s.sound == "Kamehameha" then soundFile = BL_SOUNDS["Kamehameha"] end 
    
    if soundFile then
        local channel = s.soundChannel or "Master" 
        local _, handle = PlaySoundFile(soundFile, channel)
        BloodlustState.soundHandle = handle
    end
end

function Screen.PreviewBloodlust()
    Screen.TriggerBloodlust("Test Player")
end

function Screen.ToggleBloodlustMover()
    if not BloodlustState.frame then CreateBloodlustFrame() end
    local f = BloodlustState.frame
    
    -- Update visuals to match current settings so user sees what they are moving
    local s = GetBloodlustSettings()
    if s then
        local style = s.texture or "goku"
        local media = BL_MEDIA[style] or BL_MEDIA["goku"]
        ...
        if media then
            ...
            if media.type == "flipbook" then
                -- Show first frame
                local left = 0
                local right = 1 / media.cols
                local top = 0
                local bottom = 1 / media.rows
                BloodlustState.texture:SetTexCoord(left, right, top, bottom)
            else
                BloodlustState.texture:SetTexCoord(0, 1, 0, 1)
            end
        else
            BloodlustState.texture:SetTexture("Interface/AddOns/GravityUI/assets/media/BL/goku.tga")
            BloodlustState.texture:SetTexCoord(0, 1, 0, 1)
    ...
    if ns.Movers and ns.Movers.Toggle then
        ns.Movers:Toggle("GravityUI_Bloodlust")
    else
        if f:IsShown() then
            f:Hide()
            f:EnableMouse(false)
            ...
            f:EnableMouse(true) 
            f:SetMovable(true)
            if not f:GetScript("OnDragStart") then
...
-- Event Handler
local function OnSpellCast(self, event, unit, castGUID, spellID)
    -- LAUNDER SPELL ID
    ...
    if not BLOODLUST_SPELLS[spellID] then return end
    
    -- Filter: Group or Player
    if UnitInParty(unit) or UnitInRaid(unit) or UnitIsUnit(unit, "player") then
        local name = UnitName(unit)
        Screen.TriggerBloodlust(name)
    end
end

local blListener = CreateFrame("Frame")
