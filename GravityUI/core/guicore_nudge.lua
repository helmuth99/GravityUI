...
local function IsNudgeTargetFrameName(frameName)
    ...
    if guiCore.viewers then
        for _, viewerName in ipairs(guiCore.viewers) do
...
local function CreateNudgeUI()
    ...
    local function CreateArrowButton(parent, direction, x, yFromTop)
        ...
        button:SetScript("OnClick", function()
            guiCore:NudgeSelectedViewer(direction)
    ...
    amountSlider:SetScript("OnValueChanged", function(self, value)
        ...
        guiCore.db.profile.nudgeAmount = value
    ...
    local function ViewerDropdown_Initialize(self, level)
        ...
        if guiCore.viewers then
            for _, viewerName in ipairs(guiCore.viewers) do
                ...
                info.func = function()
                    guiCore:SelectViewer(viewerName)
                    UIDropDownMenu_SetText(viewerDropdown, displayName)
                    CloseDropDownMenus()
                end
                info.checked = (guiCore.selectedViewer == viewerName)
        ...
        for frameName, label in pairs(UNIT_ANCHOR_FRAMES) do
            ...
            info.func = function()
                guiCore:SelectViewer(frameName)
                UIDropDownMenu_SetText(viewerDropdown, displayName)
                CloseDropDownMenus()
            end
            info.checked = (guiCore.selectedViewer == frameName)
            UIDropDownMenu_AddButton(info, level)
        end
    end
    ...
    function NudgeFrame:UpdateInfo()
        local viewerName = guiCore.selectedViewer
    ...
    function NudgeFrame:UpdateAmountSlider()
        local amount = guiCore.db.profile.nudgeAmount or 1
...
local function CreateMinimapNudgeButton(parent, direction)
    ...
    btn:SetScript("OnClick", function()
        guiCore:SelectEditModeElement("minimap", "minimap")
        guiCore:NudgeMinimap(direction)
    end)
...
-- Create overlay for the gui minimap
...
function guiCore:ShowViewerOverlays()
...
function guiCore:HideViewerOverlays()
    for _, viewerName in ipairs(CDM_VIEWERS) do
        local overlay = viewerOverlays[viewerName]
        if overlay then
            overlay:Hide()
...
function guiCore:ShowBlizzardFrameOverlays()
...
function guiCore:HideBlizzardFrameOverlays()
    for _, frameInfo in ipairs(BLIZZARD_EDITMODE_FRAMES) do
...
function guiCore:ShowMinimapOverlay()
    ...
    if minimapOverlay then
        ...
        minimapOverlay:SetScript("OnMouseUp", function(self, button)
            ...
            -- Save position to DB
            local settings = guiCore.db and guiCore.db.profile and guiCore.db.profile.minimap
            if settings then
...
function guiCore:HideMinimapOverlay()
    if minimapOverlay then
        minimapOverlay:Hide()
...
function guiCore:EnableClickDetection()
    clickDetector:Show()
                    ...
                    if frame and frame ~= WorldFrame then
                        ...
                        if not frameName and frame:GetParent() then
                            ...
                            if IsNudgeTargetFrameName(parentName) then
                                if lastClickedFrame ~= frame then
                                    lastClickedFrame = frame
                                    guiCore:SelectViewer(parentName)
                                end
                                return
                            end
                        end
...
function guiCore:DisableClickDetection()
    clickDetector:Hide()
...
function guiCore:SelectViewer(viewerName)
...
function guiCore:NudgeSelectedViewer(direction)
...
-- Nudge the minimap
function guiCore:NudgeMinimap(direction)
...
local function SetupEditModeHooks()
    ...
    hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
        ...
        if guiCore.nudgeFrame then
            guiCore.nudgeFrame:UpdateVisibility()
        end
        guiCore:EnableClickDetection()
        -- Let Blizzard's native Edit Mode handle CDM viewers and standard Edit Mode frames
        -- guiCore:ShowViewerOverlays()
        -- guiCore:ShowBlizzardFrameOverlays()
        guiCore:ShowMinimapOverlay()  -- Show nudge overlay on gui minimap
        guiCore:EnableMinimapEditMode()  -- Temporarily allow minimap movement
    end)

    hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
        -- NudgeFrame is lazy-loaded, only hide if it exists
        if guiCore.nudgeFrame then
            guiCore.nudgeFrame:Hide()
        end
        guiCore:DisableClickDetection()
        -- guiCore:HideViewerOverlays()
        -- guiCore:HideBlizzardFrameOverlays()
        guiCore:HideMinimapOverlay()  -- Hide minimap overlay
        guiCore:DisableMinimapEditMode()  -- Restore minimap lock setting
        guiCore.selectedViewer = nil
        -- Clear central selection (in case a CDM viewer was selected)
        if guiCore.ClearEditModeSelection then
...
-- Add nudgeamount
local oldOnInitialize = guiCore.OnInitialize
