...
local function GetDB()
    ...
    return guiCore.db.profile.actionBars
end
...
local function AddKeybindMethods(button, barKey)
    ...
    function button:GetBindings()
        ...
        for i = 1, select("#", GetBindingKey(self._guiBindingCommand)) do
            local key = select(i, GetBindingKey(self._guiBindingCommand))
            if key then
    ...
    function button:ClearBindings()
        ...
        while GetBindingKey(self._guiBindingCommand) do
            SetBinding(GetBindingKey(self._guiBindingCommand), nil)
        end
    end
...
local function GetExtraButtonDB(buttonType)
    local guiCore = _G.GravityUI and _G.GravityUI.guiCore
    if not guiCore or not guiCore.db or not guiCore.db.profile then return nil end
    return guiCore.db.profile.actionBars and guiCore.db.profile.actionBars.bars
        and guiCore.db.profile.actionBars.bars[buttonType]
end
...
local function CreateExtraButtonHolder(buttonType, displayName)
    ...
    -- Create holder frame
    local holder = CreateFrame("Frame", "gui_" .. buttonType .. "Holder", UIParent)
    ...
    local mover = CreateFrame("Frame", "gui_" .. buttonType .. "Mover", holder, "BackdropTemplate")
    ...
    mover:SetBackdropColor(0.2, 0.8, 0.6, 0.5)  -- gui mint color
...
local function HookExtraButtonPositioning()
    ...
    if ExtraActionBarFrame and not ExtraActionBarFrame._guiHooked then
        ExtraActionBarFrame._guiHooked = true
    ...
    if ZoneAbilityFrame and not ZoneAbilityFrame._guiHooked then
        ZoneAbilityFrame._guiHooked = true
    ...
    -- Only do this if gui is actually managing these frames (at least one feature enabled)
...
_G.GravityUI_ToggleExtraButtonMovers = ToggleExtraButtonMovers
_G.GravityUI_RefreshExtraButtons = RefreshExtraButtons
...
local function StripBlizzardArtwork(button)
    if button._guiStripped then return end
...
-- Apply gui skin to a single button
local function SkinButton(button, settings)
    ...
    if button._guiSkinKey == settingsKey then return end
    button._guiSkinKey = settingsKey
    ...
    if settings.showBackdrop then
        ...
        button._guiBackdrop:ClearAllPoints()
        button._guiBackdrop:SetAllPoints(button)  -- Same size as button, not extending beyond
...
-- One-time migration: if gui lockButtons was true, apply it to Blizzard CVar
-- This preserves existing user settings after the fix that stops gui from overwriting Blizzard's setting
...
-- NOTE: No longer overwrites Blizzard's CVar - gui options panel now syncs directly with it
...
local function ResetAllButtonTints()
    for i = 1, 8 do
        ...
        for _, button in ipairs(buttons) do
            local icon = button.icon or button.Icon
            if icon and button._guiTinted then
                icon:SetVertexColor(1, 1, 1, 1)
                icon:SetDesaturated(false)
                button._guiTinted = nil
            end
...
local function HookFrameForMouseover(frame, barKey)
    if not frame or frame._guiMouseoverHooked then return end
    frame._guiMouseoverHooked = true
...
local function SkinBar(barKey)
    ...
    for _, button in ipairs(buttons) do
        ...
        if not button._guiOnEnterHooked then
            button._guiOnEnterHooked = true
...
local function ApplyPageArrowVisibility(hide)
    ...
    if hide then
        pageNum:Hide()
        if not pageNum._gui_ShowHooked then
            pageNum._gui_ShowHooked = true
...
_G.GravityUI_ApplyPageArrowVisibility = ApplyPageArrowVisibility
...
_G.GravityUI_RefreshActionBars = function()
    if InCombatLockdown() then
