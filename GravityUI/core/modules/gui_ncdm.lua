...
local function PreventAtlasBorder(texture)
    if not texture or texture.__guiAtlasBlocked then return end
    texture.__guiAtlasBlocked = true
...
combatEndFrame:SetScript("OnEvent", function()
    ...
    C_Timer.After(0.1, function()
        if not InCombatLockdown() and _G.GravityUI_RefreshNCDM then
...
local function LayoutViewer(viewerName, trackerKey)
    ...
    if not viewer.__cdmUpdatePending then
        ...
        C_Timer.After(0.05, function()
            ...
            if trackerKey == "essential" then
                if _G.GravityUI_UpdateLockedPowerBar then
                    _G.GravityUI_UpdateLockedPowerBar()
                end
                if _G.GravityUI_UpdateLockedSecondaryPowerBar then
                    _G.GravityUI_UpdateLockedSecondaryPowerBar()
                end
                if _G.GravityUI_UpdateLockedCastbarToEssential then
                    _G.GravityUI_UpdateLockedCastbarToEssential()
                end
            elseif trackerKey == "utility" then
                if _G.GravityUI_UpdateLockedPowerBarToUtility then
                    _G.GravityUI_UpdateLockedPowerBarToUtility()
                end
                if _G.GravityUI_UpdateLockedSecondaryPowerBarToUtility then
                    _G.GravityUI_UpdateLockedSecondaryPowerBarToUtility()
                end
                if _G.GravityUI_UpdateLockedCastbarToUtility then
                    _G.GravityUI_UpdateLockedCastbarToUtility()
                end
            end
            -- Unit frames anchored to CDM (can be anchored to either Essential or Utility)
            if _G.GravityUI_UpdateCDMAnchoredUnitFrames then
                _G.GravityUI_UpdateCDMAnchoredUnitFrames()
            end
    ...
    viewer:HookScript("OnShow", function(self)
        ...
        C_Timer.After(0.02, function()
            if self:IsShown() then
                ...
                if trackerKey == "utility" and _G.GravityUI_ApplyUtilityAnchor then
                    _G.GravityUI_ApplyUtilityAnchor()
                end
            end
        end)
...
_G.GravityUI_RefreshNCDM = RefreshAll
_G.GravityUI_IncrementNCDMVersion = IncrementSettingsVersion
_G.GravityUI_ApplyUtilityAnchor = ApplyUtilityAnchor
...
local function GetCDMVisibilitySettings()
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.cdmVisibility then
        return guiCore.db.profile.cdmVisibility
    end
    return nil
end
...
local function GetUnitframesVisibilitySettings()
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.unitframesVisibility then
        return guiCore.db.profile.unitframesVisibility
    end
    return nil
end
...
local function GetUnitframeFrames()
    ...
    -- Collect unit frames
    if _G.GravityUI_UnitFrames then
        for unitKey, frame in pairs(_G.GravityUI_UnitFrames) do
            if frame then
...
local function UpdateUnitframesVisibility()
    ...
    if _G.GravityUI_Castbars then
        ...
        if vis and vis.alwaysShowCastbars then
            ...
            if _G.GravityUI_UnitFrames then
                for _, frame in pairs(_G.GravityUI_UnitFrames) do
                    if frame then
...
local function SetupUnitframesMouseoverDetector()
    ...
    for _, frame in ipairs(ufFrames) do
        if frame and not frame._guiMouseoverHooked then
            frame._guiMouseoverHooked = true
...
_G.GravityUI_RefreshCDMVisibility = UpdateCDMVisibility
_G.GravityUI_RefreshUnitframesVisibility = UpdateUnitframesVisibility
_G.GravityUI_RefreshCDMMouseover = SetupCDMMouseoverDetector
_G.GravityUI_RefreshUnitframesMouseover = SetupUnitframesMouseoverDetector
