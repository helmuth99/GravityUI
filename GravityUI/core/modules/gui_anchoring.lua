...
-- SETUP HELPERS
---------------------------------------------------------------------------
function gui_Anchoring:SetHelpers(helpers)
    Helpers = helpers or {}
end
...
function gui_Anchoring:GetAnchorDimensions(anchorFrame, anchorTargetName)
...
function gui_Anchoring:PositionFrame(frame, anchorTarget, anchorPoint, offsetX, offsetY, parentFrame, options)
...
function gui_Anchoring:GetAnchorTargetName(frame)
...
function gui_Anchoring:CheckCircularDependency(frame, anchorTarget)
...
function gui_Anchoring:RegisterAnchoredFrame(frame, config)
    ...
    if frame._guiReRegisterStateDriver then
        C_Timer.After(0, function()
            if frame and frame._guiReRegisterStateDriver then
                frame._guiReRegisterStateDriver()
            end
        end)
    end
...
function gui_Anchoring:UnregisterAnchoredFrame(frame)
    if not frame then return false end
...
function gui_Anchoring:SnapTo(frame, anchorTarget, anchorPoint, offsetX, offsetY, options)
    ...
    -- Re-register state drivers for unit frames after positioning (ClearAllPoints breaks them)
    if success and frame._guiReRegisterStateDriver then
        C_Timer.After(0, function()
            if frame and frame._guiReRegisterStateDriver then
                frame._guiReRegisterStateDriver()
            end
...
function gui_Anchoring:UpdateAllAnchoredFrames()
    if InCombatLockdown() then 
...
function gui_Anchoring:UpdateFramesForTarget(anchorTargetName)
    if InCombatLockdown() then 
        -- Defer update after combat
        C_Timer.After(0, function()
...
_G.GravityUI_UpdateAnchoredFrames = function()
    if gui_Anchoring then
        gui_Anchoring:UpdateAllAnchoredFrames()
    end
end

-- Backward compatibility alias
_G.GravityUI_UpdateAnchoredUnitFrames = _G.GravityUI_UpdateAnchoredFrames
_G.GravityUI_UpdateCDMAnchoredUnitFrames = _G.GravityUI_UpdateAnchoredFrames
