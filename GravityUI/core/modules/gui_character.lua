---------------------------------------------------------------------------
-- GravityUI Character Pane Module
...
local function GetSettings()
    ...
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.character then
        return guiCore.db.profile.character
    end
...
local function IsSkinningHandlingBackground()
    local skinningAPI = _G.gui_CharacterFrameSkinning
...
ScheduleUpdate = function()
    ...
    C_Timer.After(0.05, function()
        ...
        if ns.gui.InspectPane and ns.gui.InspectPane.UpdateInspectFrame then
            ns.gui.InspectPane.UpdateInspectFrame()
        end
    end)
end
...
local function HookCharacterFrame()
    ...
    local function HideCustomElements()
        ...
        if CharacterFrame._guiILvlDisplay then CharacterFrame._guiILvlDisplay:Hide() end
        if CharacterFrame._guiCenterILvl then CharacterFrame._guiCenterILvl:Hide() end
        if CharacterFrame._guiGearBtn then CharacterFrame._guiGearBtn:Hide() end
        if CharacterFrame._guiSettingsPanel then CharacterFrame._guiSettingsPanel:Hide() end
    ...
    if PaperDollFrame then
        PaperDollFrame:HookScript("OnShow", function()
            ...
            if settings.enabled then
                ...
                if CharacterFrame._guiGearBtn then CharacterFrame._guiGearBtn:Show() end
                ScheduleUpdate()
    ...
    if not settings.enabled then
        ...
        if CharacterFrame._guiGearBtn then
            CharacterFrame._guiGearBtn:Hide()
        end
        if CharacterFrame._guiSettingsPanel then
            CharacterFrame._guiSettingsPanel:Hide()
        end
        return
    end

    -- Create gear icon (more prominent position in title bar)
    if not CharacterFrame._guiGearBtn then
        gearBtn = CreateFrame("Button", "gui_CharacterSettingsBtn", CharacterFrame, "BackdropTemplate")
        ...
        CharacterFrame._guiGearBtn = gearBtn

        -- Settings panel (positioned to the right of CharacterFrame)
        settingsPanel = CreateFrame("Frame", "GravityUI_CharSettingsPanel", CharacterFrame, "BackdropTemplate")
        ...
        local GUI = _G.GravityUI and _G.GravityUI.GUI
        ...
        local function RefreshAll()
            if _G.GravityUI_RefreshCharacterPanelFonts then
                _G.GravityUI_RefreshCharacterPanelFonts()
            end
            ScheduleUpdate()		
        end
        ...
        local guiCore = _G.GravityUI and _G.GravityUI.guiCore
        local generalDB = guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.general
        ...
        if generalDB then
            ...
            -- Refresh color picker when panel shows (in case color changed in main gui options)
...
---------------------------------------------------------------------------
_G.GravityUI_RefreshCharacterPane = function()
    ScheduleUpdate()
end
...
ns.CharacterPane = gui.CharacterPane

---------------------------------------------------------------------------
-- Shared exports for gui_inspect.lua
