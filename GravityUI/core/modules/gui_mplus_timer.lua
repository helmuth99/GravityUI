...
local function GetSettings()
    ...
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.mplusTimer then
        return guiCore.db.profile.mplusTimer
    end
    return {
        enabled = true,
...
local function GetPosition()
    local defaults = { point = "TOPRIGHT", relPoint = "TOPRIGHT", x = -100, y = -200 }
     ...
     local guiCore = _G.GravityUI and _G.GravityUI.guiCore
     if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.mplusTimer then
         local pos = guiCore.db.profile.mplusTimer.position
        if pos then
            return {
...
local function SavePosition(point, relPoint, x, y)
    ...
    if guiCore and guiCore.db and guiCore.db.profile then
        if not guiCore.db.profile.mplusTimer then
            guiCore.db.profile.mplusTimer = {}
        end
        guiCore.db.profile.mplusTimer.position = {
            point = point,
            relPoint = relPoint,
            x = x,
            y = y
...
function MPlusTimer:CreateFrames()
    ...
    -- Root frame
    local root = CreateFrame("Frame", "gui_MPlusTimerFrame", UIParent)
...
function MPlusTimer:EnableDemoMode()
    if self.state.inChallenge then
        print("|cFF34D4E8[GUI M+ Timer]|r Can't enable demo mode during active M+!")
        return
    end
    ...
    print("|cFF34D4E8[GUI M+ Timer]|r Demo mode enabled. Type /gmpt demo to disable.")
end
...
function MPlusTimer:DisableDemoMode()
    ...
    print("|cFF34D4E8[GUI M+ Timer]|r Demo mode disabled.")
end
...
-- Slash Command
---------------------------------------------------------------------------
SLASH_GUIIMPLUSTIMER1 = "/gmpt"
SlashCmdList["GUIIMPLUSTIMER"] = function(msg)
    ...
    elseif cmd == "hide" then
        ...
        print("|cFF34D4E8[GUI M+ Timer]|r Commands:")
        print("  /gmpt demo - Toggle demo mode")
        print("  /gmpt show - Show timer")
        print("  /gmpt hide - Hide timer")
    end
end
...
-- Expose for skinning
---------------------------------------------------------------------------
_G.GravityUI_MPlusTimer = MPlusTimer
