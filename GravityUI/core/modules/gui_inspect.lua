---------------------------------------------------------------------------
-- GravityUI Inspect Pane Module
-- Custom inspect panel styling with equipment overlays and stats panel
-- Split from gui_character.lua for better organization
---------------------------------------------------------------------------
local ADDON_NAME, ns = ...
local gui = ns.gui or {}
ns.gui = gui
...
---------------------------------------------------------------------------
-- Import shared functions from gui_character.lua
-- These will be available after gui_character.lua loads
---------------------------------------------------------------------------
local function GetShared()
    return ns.gui.CharacterShared or {}
end
...
local function SetupInspectTitleArea()
    ...
    if not InspectFrame._guiILvlDisplay then
        ...
        InspectFrame._guiILvlDisplay = displayFrame
    end
    ...
    if not InspectFrame._guiCenterILvl then
        ...
        InspectFrame._guiCenterILvl = centerFrame
    end
end
...
local function UpdateInspectILvlDisplay()
    ...
    local displayFrame = InspectFrame._guiILvlDisplay
    ...
    local centerFrame = InspectFrame._guiCenterILvl
...
local function SetInspectExtendedMode(tabNum)
    ...
    if _G.gui_InspectFrameSkinning and _G.gui_InspectFrameSkinning.SetExtended then
        _G.gui_InspectFrameSkinning.SetExtended(true)
    end
    -- Show iLvl display on Character/PvP tabs
    if InspectFrame._guiCenterILvl then
        InspectFrame._guiCenterILvl:Show()
    end
end
...
local function SetInspectNormalMode()
    ...
    if _G.gui_InspectFrameSkinning and _G.gui_InspectFrameSkinning.SetExtended then
        _G.gui_InspectFrameSkinning.SetExtended(false)
    end
    -- Hide iLvl display on Guild tab (not relevant)
    if InspectFrame._guiCenterILvl then
        InspectFrame._guiCenterILvl:Hide()
    end
end
...
local function CreateInspectSettingsButton()
    if not InspectFrame then return end
    if InspectFrame._guiGearBtn then return end

    local GUI = _G.GravityUI and _G.GravityUI.GUI
    if not GUI then return end

    local guiCore = _G.GravityUI and _G.GravityUI.guiCore
    if not (guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.character) then
        C_Timer.After(0.5, CreateInspectSettingsButton)
        return
    end
    local charDB = guiCore.db.profile.character
    ...
    local gearBtn = CreateFrame("Button", "gui_InspectSettingsBtn", InspectFrame, "BackdropTemplate")
    ...
    InspectFrame._guiGearBtn = gearBtn

    -- Settings panel (matches character panel size: 450x600)
    inspectSettingsPanel = CreateFrame("Frame", "GravityUI_InspectSettingsPanel", InspectFrame, "BackdropTemplate")
    ...
    InspectFrame._guiSettingsPanel = inspectSettingsPanel
    ...
    title:SetText("GUI Inspect Panel")
    ...
    if generalDB then
        bgColorPicker = GUI:CreateFormColorPicker(scrollChild, "Background Color", "skinBgColor", generalDB, function()
            ...
            if _G.gui_InspectFrameSkinning and _G.gui_InspectFrameSkinning.Refresh then
                _G.gui_InspectFrameSkinning.Refresh()
            end
        end)
...
---------------------------------------------------------------------------
-- Update inspect frame (called from gui_character.lua's ScheduleUpdate)
---------------------------------------------------------------------------
...
---------------------------------------------------------------------------
-- Module API (exported for gui_character.lua to call)
---------------------------------------------------------------------------
gui.InspectPane = {
...
ns.InspectPane = gui.InspectPane
