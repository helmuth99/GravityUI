...
local function GetSettings()
    ...
    if not guiCore.db.profile.uiHider then
    ...
    local uiHider = guiCore.db.profile.uiHider
...
local function ApplyHideSettings()
    ...
    if GameTimeFrame then
        ...
        if not GameTimeFrame._gui_ShowHooked then
            GameTimeFrame._gui_ShowHooked = true
    ...
    if StatusTrackingBarManager then
        ...
        if hideXP and hideRep then
            ...
            if not StatusTrackingBarManager._gui_ShowHooked then
                StatusTrackingBarManager._gui_ShowHooked = true
        ...
        elseif hideXP or hideRep then
            ...
            if not StatusTrackingBarManager._gui_BarsHooked then
                StatusTrackingBarManager._gui_BarsHooked = true
                hooksecurefunc(StatusTrackingBarManager, "UpdateBarsShown", function()
                    C_Timer.After(0.01, HideStatusBars)
                end)
            end
        end
        -- else: Do nothing - let other addons/Blizzard manage the bar when gui isn't hiding
    end
...
-- Initialize
-- Note: ApplyHideSettings is now called from guicore_main.lua OnEnable()
