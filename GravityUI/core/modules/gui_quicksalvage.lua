...
local function GetSettings()
    ...
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.general then
        return guiCore.db.profile.general.quickSalvage
    end
    return nil
end
...
local SalvageButton = CreateFrame("Button", "GravityUI_QuickSalvageButton", UIParent, table.concat(TEMPLATES, ','))
...
---------------------------------------------------------------------------
function _G.GravityUI_RefreshQuickSalvage()
    if not InCombatLockdown() then
        SalvageButton:UpdateAttributeDriver()
    end
end
...
QuickSalvage.Refresh = _G.GravityUI_RefreshQuickSalvage
