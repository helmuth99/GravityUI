...
local function GetLockoutCacheTTL()
    local db = guiCore.db and guiCore.db.profile and guiCore.db.profile.datatext
...
function Datatexts:Register(id, datatextDef)
    if self.registry[id] then
        print("|cffff0000GravityUI:|r Datatext '" .. id .. "' is already registered!")
        return false
    end
    
    if not datatextDef.OnEnable or type(datatextDef.OnEnable) ~= "function" then
        print("|cffff0000GravityUI:|r Datatext '" .. id .. "' missing OnEnable function!")
        return false
    end
...
function Datatexts:AttachToSlot(slotFrame, datatextID, settings)
    if not slotFrame then
        print("|cffff0000GravityUI:|r Invalid slot frame provided")
        return false
    end
    ...
    if not success then
        print("|cffff0000GravityUI:|r Failed to enable datatext '" .. datatextID .. "': " .. tostring(instance))
        return false
    end
...
Datatexts:Register("time", {
    ...
    OnEnable = function(slotFrame, settings)
        ...
        local function Update()
            ...
            local dtSettings = guiCore.db and guiCore.db.profile and guiCore.db.profile.datatext
            ...
            MenuUtil.CreateContextMenu(anchorFrame, function(_, root)
                ...
                for charKey, charData in pairs(db.global.goldData) do
                    ...
                    local btn = root:CreateButton(colorCode .. charKey .. "|r - " .. FormatGold(charMoney), function()
                        -- Confirm deletion
                        StaticPopupDialogs["Gravity_GOLD_DELETE_CHAR"] = {
                            text = "Delete gold data for " .. deleteCharKey .. "?",
                            button1 = "Delete",
                            button2 = "Cancel",
                            OnAccept = function()
                                db.global.goldData[deleteCharKey] = nil
                                print("|cff30D1FF[GravityUI]|r Removed gold data for " .. deleteCharKey)
                            end,
                            timeout = 0,
                            whileDead = true,
                            hideOnEscape = true,
                        }
                        StaticPopup_Show("Gravity_GOLD_DELETE_CHAR")
                    end)
                ...
                root:CreateButton("|cffFF6666Reset All (Keep Current)|r", function()
                    StaticPopupDialogs["Gravity_GOLD_RESET_ALL"] = {
                        ...
                        OnAccept = function()
                            ...
                            print("|cff30D1FF[GravityUI]|r Reset gold data (kept current character)")
                        end,
                        timeout = 0,
                        whileDead = true,
                        hideOnEscape = true,
                    }
                    StaticPopup_Show("Gravity_GOLD_RESET_ALL")
                end)
            end)
        end
...
Datatexts:Register("guild", {
    ...
    OnEnable = function(slotFrame, settings)
        ...
        local function Update()
			...
			local displayTitle = (guildName or "Gilde") .. ": "
			local label = GetLabel(displayTitle, "G: ", slotFrame.shortLabel, slotFrame.noLabel)

            if settings.showGuildName then

                if guildName then
					text:SetFormattedText("%s|cff%02x%02x%02x%d|r", label, r, g, b, online or 0)
                else
                    text:SetFormattedText("%s|cff%02x%02x%02x%d|r", label, r, g, b, online or 0)
                end
...
-- Use shared dungeon data from gui_dungeon_data.lua
local function GetShortDungeonName(mapID)
    if _G.gui_DungeonData then
        return _G.gui_DungeonData.GetShortName(mapID)
    end
                            ...
                            if specName then
                                ...
                                root:CreateButton(iconText .. " " .. specName .. (isActive and activeMarker or ""), function()
                                    if InCombatLockdown() then
                                        print("|cffFF6B6BGravityUI:|r Cannot change specialization in combat")
                                        return
                                    end
