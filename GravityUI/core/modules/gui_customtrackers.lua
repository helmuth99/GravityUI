...
-- Will be set during initialization when guiCore is available
local guiCore
...
local function GetRechargeEdgeSetting()
    local core = _G.GravityUI and _G.GravityUI.guiCore
...
local function GetDB()
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.customTrackers then
        return guiCore.db.profile.customTrackers
    end
    return nil
end

local function GetGlobalDB()
    if guiCore and guiCore.db and guiCore.db.global then
        return guiCore.db.global
    end
    return nil
end
...
local function GetSpellBuffInfo(spellID)
    ...
    local scanner = gui and gui.SpellScanner
...
local PROC_GLOW_MASK = "Interface\\AddOns\\GravityUI\\assets\\iconskin\\ProcGlowMask"
...
local function StartActiveGlow(icon, config)
    ...
    elseif glowType == "Pixel Glow" then
        LCG.PixelGlow_Start(icon, color, lines, frequency, nil, thickness, 0, 0, true, "_guiActiveGlow")
        icon._activeGlowShown = true
        icon._activeGlowType = glowType
    elseif glowType == "Autocast Shine" then
        LCG.AutoCastGlow_Start(icon, color, lines, frequency, scale, 0, 0, "_guiActiveGlow")
        icon._activeGlowShown = true
        icon._activeGlowType = glowType
    end
end
...
local function StopActiveGlow(icon)
    ...
    if glowType == "Proc Glow" then
        pcall(LCG.ProcGlow_Stop, icon, "_guiActiveGlow")
    ...
    elseif glowType == "Pixel Glow" then
        pcall(LCG.PixelGlow_Stop, icon, "_guiActiveGlow")
    elseif glowType == "Autocast Shine" then
        pcall(LCG.AutoCastGlow_Stop, icon, "_guiActiveGlow")
    end
...
local function CreateTrackerIcon(parent)
    ...
    icon:SetScript("OnEnter", function(self)
        ...
        if self.entry then
            ...
            local tooltipSettings = gui.guiCore and gui.guiCore.db and gui.guiCore.db.profile and gui.guiCore.db.profile.tooltip
...
local function ApplyKeybindToTrackerIcon(icon)
    ...
    local guiKeybinds = ns and ns.Keybinds
    if not guiKeybinds then
        if icon.keybindText then
            icon.keybindText:Hide()
        end
        return
    ...
    if entry.type == "spell" and entry.id then
        keybind = guiKeybinds.GetKeybindForSpell(entry.id)
        -- Try spell name fallback if no keybind found
        if not keybind and guiKeybinds.GetKeybindForSpellName then
            local spellInfo = C_Spell.GetSpellInfo(entry.id)
            if spellInfo and spellInfo.name then
                keybind = guiKeybinds.GetKeybindForSpellName(spellInfo.name)
            end
        end
    elseif entry.type == "item" and entry.id then
        keybind = guiKeybinds.GetKeybindForItem(entry.id)
        -- Try item name fallback if no keybind found
        if not keybind and guiKeybinds.GetKeybindForItemName then
            local itemName = C_Item.GetItemInfo(entry.id)
            if itemName then
                keybind = guiKeybinds.GetKeybindForItemName(itemName)
            end
        end
    end
...
local function RebuildActiveSet(bar)
    ...
    -- print("|cFF00FF00[GUI Debug]|r RebuildActiveSet: " .. #bar.activeIcons .. " of " .. #(bar.icons or {}) .. " icons active")
end
...
function CustomTrackers:CreateBar(barID, config)
    ...
    local bar = CreateFrame("Frame", "gui_CustomTracker_" .. barID, UIParent, "BackdropTemplate")
...
---------------------------------------------------------------------------
_G.GravityUI_RefreshCustomTrackers = function()
    -- Use local CustomTrackers directly since guiCore might not be set yet
...
initFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    if event == "PLAYER_ENTERING_WORLD" then
        -- Set guiCore reference now that it's available
        guiCore = gui.guiCore
        if guiCore then
            guiCore.CustomTrackers = CustomTrackers
        end
...
local function GetCustomTrackersVisibilitySettings()
    local db = gui.guiCore and gui.guiCore.db
...
---------------------------------------------------------------------------
_G.GravityUI_RefreshCustomTrackersVisibility = UpdateCustomTrackersVisibility
_G.GravityUI_RefreshCustomTrackersMouseover = SetupCustomTrackersMouseoverDetector

-- Refresh keybind display on all custom tracker icons
_G.GravityUI_RefreshCustomTrackerKeybinds = function()
