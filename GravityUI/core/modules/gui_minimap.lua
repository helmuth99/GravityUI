...
guiCore.Minimap = Minimap_Module
...
-- Datatext panel (3-slot architecture using guiCore.Datatexts registry)
local datatextFrame
...
local function GetSettings()
    ...
    cachedSettings = guiCore.db.profile.minimap
...
local function CreateBackdrop()
    ...
    backdropFrame = CreateFrame("Frame", "gui_MinimapBackdrop", Minimap)
...
local function GetDatatextSettings()
    ...
    return guiCore.db.profile.datatext
end
...
local function CreateDatatextPanel()
    ...
    datatextFrame = CreateFrame("Frame", "gui_DatatextPanel", UIParent)
...
local function RefreshDatatextSlots()
    ...
    if not guiCore or not guiCore.Datatexts then return end
    ...
    for i, slot in ipairs(datatextFrame.slots) do
        ...
        if datatextID and datatextID ~= "" then
            ...
            -- Attach datatext
            guiCore.Datatexts:AttachToSlot(slot, datatextID, dtSettings)
        else
            -- Empty slot: hide
            slot:Hide()
            slot.text:SetText("")
...
local function UpdateClock()
    ...
    guiCore:SafeSetFont(clockText, fontPath, clockConfig.fontSize, flags)
...
local function UpdateCoords()
    ...
    guiCore:SafeSetFont(coordsText, fontPath, coordsConfig.fontSize, flags)
...
local function UpdateZoneText()
    ...
    local fontPath = LSM:Fetch("font", generalFont) or "Fonts\\FRIZQT__.TTF"
    guiCore:SafeSetFont(zoneTextFont, fontPath, zoneConfig.fontSize, flags)
...
if Minimap.ZoomIn and not Minimap.ZoomIn._gui_ShowHooked then
    Minimap.ZoomIn._gui_ShowHooked = true
...
if Minimap.ZoomOut and not Minimap.ZoomOut._gui_ShowHooked then
    Minimap.ZoomOut._gui_ShowHooked = true
...
function guiCore:EnableMinimapEditMode()
    local settings = GetSettings()
...
function guiCore:DisableMinimapEditMode()
    local settings = GetSettings()
    if not settings then return end
...
function Minimap_Module:Refresh()
    ...
    if not settings.enabled then
        StopUpdateTickers()
        -- Hide gui customizations but keep minimap visible
        if minimapBackdrop then
            minimapBackdrop:Hide()
        end
