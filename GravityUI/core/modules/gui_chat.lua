...
local function GetSettings()
    ...
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.chat then
        return guiCore.db.profile.chat
    end
    return nil
end
...
---------------------------------------------------------------------------
-- Create URL copy popup (on demand) - gui styled
---------------------------------------------------------------------------
local function CreateCopyPopup()
    ...
    -- gui color scheme
    urlPopup:SetBackdropColor(gui_COLORS.bg[1], gui_COLORS.bg[2], gui_COLORS.bg[3], gui_COLORS.bg[4])
    urlPopup:SetBackdropBorderColor(gui_COLORS.accent[1], gui_COLORS.accent[2], gui_COLORS.accent[3], gui_COLORS.accent[4])
    ...
    title:SetTextColor(gui_COLORS.accent[1], gui_COLORS.accent[2], gui_COLORS.accent[3], 1)
    ...
    editBox:SetTextColor(gui_COLORS.text[1], gui_COLORS.text[2], gui_COLORS.text[3], 1)
...
local function CreateChatCopyFrame()
    ...
    chatCopyFrame:SetBackdropColor(gui_COLORS.bg[1], gui_COLORS.bg[2], gui_COLORS.bg[3], gui_COLORS.bg[4])
    chatCopyFrame:SetBackdropBorderColor(gui_COLORS.accent[1], gui_COLORS.accent[2], gui_COLORS.accent[3], gui_COLORS.accent[4])
...
local function SetupCopyButtonHoverMode(chatFrame)
    ...
    if chatFrame.GravityCopyButtonHooked then return end
...
local function StyleEditBox(chatFrame)
    ...
    if not editBox.__guiChatStyled then
        ...
        for _, region in ipairs(regions) do
            if region and region.GetObjectType and region:GetObjectType() == "Texture" then
                if not region.__guiChatKeep then
                    region:SetAlpha(0)
                end
            end
        end
    ...
    if positionTop then
        ...
        editBox.__guiChatBackdrop = backdrop
        ...
        if not editBox.__guiTopModeHooked then
            ...
            editBox:HookScript("OnEditFocusGained", function(self)
                local s = GetSettings()
                if s and s.editBox and s.editBox.positionTop and self.__guiChatBackdrop then
                    self.__guiChatBackdrop:Show()
                end
            end)
            editBox:HookScript("OnEditFocusLost", function(self)
                if self.__guiChatBackdrop then
                    self.__guiChatBackdrop:Hide()
                end
            end)
        end
...
local function UpdateTabColors(tab)
    local settings = GetSettings()
    if not settings or not tab.__guiBackdrop then return end
    ...
    if isSelected then
        ...
        tab.__guiBackdrop:SetBackdropColor(0, 0, 0, alpha + 0.2)
        tab.__guiBackdrop:SetBackdropBorderColor(gui_COLORS.accent[1], gui_COLORS.accent[2], gui_COLORS.accent[3], 1)
    else
        -- Unselected: standard glass
        tab.__guiBackdrop:SetBackdropColor(0, 0, 0, alpha)
        tab.__guiBackdrop:SetBackdropBorderColor(0, 0, 0, alpha)
    end
end
...
local function RemoveEditBoxStyle(chatFrame)
    ...
    if chatFrame.__guiEditBoxBackdrop then
        chatFrame.__guiEditBoxBackdrop:Hide()
    end
end
...
local function RefreshAll()
    ...
    for chatFrame in pairs(skinnedFrames) do
        ...
        if not settings or not settings.enabled or not settings.editBox or not settings.editBox.enabled then
            ...
            if chatFrame.__guiEditBoxBackdrop and not settings.editBox.positionTop then
                chatFrame.__guiEditBoxBackdrop:Show()
            end
        end
...
---------------------------------------------------------------------------
_G.GravityUI_RefreshChat = RefreshAll
