...
gui.FormatKeybind = FormatKeybind
...
local function ApplyKeybindToIcon(icon, viewerName)
    local guiCore = _G.GravityUI and _G.GravityUI.guiCore
    if not guiCore or not guiCore.db or not guiCore.db.profile then return end
    
    local settings = guiCore.db.profile.viewers[viewerName]
...
local function ClearStoredKeybinds(viewerName)
    ...
    for _, child in ipairs(children) do
        child._guiKeybind = nil
        child._guiKeybindSpellID = nil
    end
end
...
local function HookViewerLayout(viewerName)
    ...
    if viewer.Layout and not viewer._gui_KeybindHooked then
        viewer._gui_KeybindHooked = true
...
_G.GravityUI_UpdateViewerKeybinds = function(viewerName)
    UpdateViewerKeybinds(viewerName)
end
-- Debug function to see what's in the cache
local function DebugPrintCache()
    print("|cFF56D1FF[GUI Keybinds]|r Cache contents:")
...
local function DebugMacro(macroName)
    ...
    print("|cFF56D1FF[GUI Keybinds]|r Macro Debug: " .. name)
...
local function DebugFindMacro(macroName)
    ...
    print("|cFF56D1FF[GUI Keybinds]|r Searching for macro '" .. macroName .. "' (index " .. targetMacroIndex .. ") on action buttons...")
...
local function DebugKey(keyName)
    print("|cFF56D1FF[GUI Keybinds]|r Checking what '" .. keyName .. "' is bound to...")
...
SLASH_guiKEYBINDS1 = "/guikeybinds"
SlashCmdList["guiKEYBINDS"] = function(msg)
    if msg == "debug" then
        RebuildCache()
        DebugPrintCache()
    elseif msg == "refresh" then
        UpdateAllKeybinds()
        print("|cFF56D1FF[GUI Keybinds]|r Refreshed keybinds")
    elseif msg == "rebuild" then
        ForceRebuildButtonCache()
        RebuildCache()
        print("|cFF56D1FF[GUI Keybinds]|r Force rebuilt button and spell caches")
    ...
    elseif msg == "buttons" then
        ...
        print("|cFF56D1FF[GUI Keybinds]|r Action button cache:")
    ...
    elseif msg == "dominos" then
        ...
        print("|cFF56D1FF[GUI Keybinds]|r Scanning Dominos buttons for macros...")
        local found = 0
        for i = 1, 180 do
            local btn = _G["DominosActionButton" .. i]
            if btn then
    ...
    elseif msg == "bartender" then
        ...
        print("|cFF56D1FF[GUI Keybinds]|r Scanning Bartender4 buttons for macros...")
        local found = 0
        for i = 1, 120 do
    ...
    elseif msg:match("^trace%s+") then
        ...
        print("|cFF56D1FF[GUI Keybinds]|r Tracing button: " .. btnName)
    ...
    elseif msg == "proctest" then
        ...
        print("|cFF56D1FF[GUI Keybinds]|r Proc debug mode: " .. (KEYBIND_DEBUG and "|cFF00FF00ON|r" or "|cFFFF0000OFF|r"))
        if KEYBIND_DEBUG then
            print("  Watch chat for keybind tracking messages when spells proc")
        end
    else
        print("|cFF56D1FF[GUI Keybinds]|r Commands:")
        print("  /guikeybinds debug - Show cache contents")
        print("  /guikeybinds refresh - Force refresh keybinds")
        print("  /guikeybinds rebuild - Force rebuild button cache")
        print("  /guikeybinds macro <name> - Debug a specific macro")
        print("  /guikeybinds find <name> - Find which button has a macro")
        print("  /guikeybinds buttons - Show cached action buttons")
        print("  /guikeybinds key <key> - Check what a key is bound to")
        print("  /guikeybinds trace <button> - Trace a specific button")
        print("  /guikeybinds dominos - Scan Dominos buttons for macros")
        print("  /guikeybinds bartender - Scan Bartender4 buttons for macros")
        print("  /guikeybinds proctest - Toggle proc debug mode")
    end
end
...
local function ShouldRunRotationHelper()
    local guiCore = _G.GravityUI and _G.GravityUI.guiCore
    if not guiCore or not guiCore.db or not guiCore.db.profile then return false end
    
    local viewers = guiCore.db.profile.viewers
...
_G.GravityUI_RefreshKeybinds = UpdateAllKeybinds
_G.GravityUI_RefreshRotationHelper = RefreshRotationHelper
