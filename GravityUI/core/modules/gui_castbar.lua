...
---------------------------------------------------------------------------
local gui_Castbar = {}
ns.gui_Castbar = gui_Castbar

gui_Castbar.castbars = {}
...
-- SETUP HELPERS
---------------------------------------------------------------------------
function gui_Castbar:SetHelpers(helpers)
    Helpers = helpers or {}
end
...
local STAGE_COLORS = gui_Castbar.STAGE_COLORS
local STAGE_FILL_COLORS = gui_Castbar.STAGE_FILL_COLORS
...
function gui_Castbar:GetEmpoweredLevel()
    local playerCastbar = self.castbars["player"]
    if not playerCastbar then
        playerCastbar = _G.GravityUI_Castbars and _G.GravityUI_Castbars["player"]
    end
...
---------------------------------------------------------------------------
function gui_Castbar:CreateCastbar(unitFrame, unit, unitKey)
    local settings = GetUnitSettings(unitKey)
...
---------------------------------------------------------------------------
function gui_Castbar:SetupCastbar(castbar, unit, unitKey, castSettings)
        ...
        if spellName or channelName or isInEmpoweredHold then
            ...
            if isPlayer and self.isEmpowered then
                ...
                if self.empoweredLevelText and self.showEmpoweredLevel then
                    local currentStage, maxStages, isEmpowered = gui_Castbar:GetEmpoweredLevel()
...
function gui_Castbar:SetupTargetFocusCastbar(castbar, unit, unitKey, castSettings)
    self:SetupCastbar(castbar, unit, unitKey, castSettings)
end

function gui_Castbar:SetupPlayerCastbar(castbar, unit, unitKey, castSettings)
    self:SetupCastbar(castbar, unit, unitKey, castSettings)
end
...
-- BOSS CASTBAR SETUP
---------------------------------------------------------------------------
function gui_Castbar:SetupBossCastbar(castbar, unit, bossIndex, castSettings)
    -- Unified OnUpdate handler - handles both real casts and preview
    local function CastBar_OnUpdate(self, elapsed)
        -- Check if actually casting (real cast takes priority)
        local spellName = UnitCastingInfo(self.unit)
        ...
        if spellName and startTimeMS and endTimeMS then
            ...
            C_Timer.After(0.1, function()
                if not UnitCastingInfo(self.unit) and not UnitChannelInfo(self.unit) then
                    ...
                    local gui_UF = gui_Castbar.unitFramesModule
                    local bossFramePreviewActive = gui_UF and gui_UF.previewMode and gui_UF.previewMode["boss" .. bossIndex]
...
-- CREATE: Boss Castbar
---------------------------------------------------------------------------
function gui_Castbar:CreateBossCastbar(unitFrame, unit, bossIndex)
    ...
    local anchorFrame = CreateAnchorFrame("gui_Boss" .. bossIndex .. "_Castbar", UIParent)
    anchorFrame:SetSize(castWidth, barHeight)
        ...
        if spellName and startTimeMS and endTimeMS then
            ...
            C_Timer.After(0.1, function()
                if not UnitCastingInfo(self.unit) and not UnitChannelInfo(self.unit) then
                    local settings = GetUnitSettings(self.unitKey)
                    local gui_UF = gui_Castbar.unitFramesModule
                    local bossFramePreviewActive = gui_UF and gui_UF.previewMode and gui_UF.previewMode["boss" .. self.bossIndex]
                    if settings and settings.castbar and settings.castbar.previewMode and bossFramePreviewActive then
                        -- Show preview simulation
...
-- GLOBAL FUNCTIONS
---------------------------------------------------------------------------
gui_Castbar.unitFramesModule = nil

function gui_Castbar:SetUnitFramesModule(ufModule)
...
_G.GravityUI_ShowCastbarPreview = function(unitKey)
    local settings = GetUnitSettings(unitKey)
    ...
    local gui_UF = gui_Castbar.unitFramesModule
    if gui_UF then
        gui_UF:RefreshFrame(unitKey)
    end
end

_G.GravityUI_HideCastbarPreview = function(unitKey)
    local settings = GetUnitSettings(unitKey)
    if not settings or not settings.castbar then
        return
    end
...
---------------------------------------------------------------------------
function gui_Castbar:RefreshCastbar(castbar, unitKey, castSettings, unitFrame)
    ...
    if newCastbar then
        local gui_UF = self.unitFramesModule
        if gui_UF and gui_UF.castbars then
            gui_UF.castbars[unitKey] = newCastbar
        end
    end
end

function gui_Castbar:RefreshBossCastbar(castbar, bossKey, castSettings, unitFrame)
    if not castSettings or not unitFrame then return end
    ...
    if newCastbar then
        local gui_UF = self.unitFramesModule
        if gui_UF and gui_UF.castbars then
            gui_UF.castbars[bossKey] = newCastbar
        end
    end
end

_G.GravityUI_RefreshCastbar = function(unitKey)
    local gui_UF = gui_Castbar.unitFramesModule
    if not gui_UF then return end
    gui_UF:RefreshFrame(unitKey)
end

-- Refresh all castbars (used by HUD Layering options)
_G.GravityUI_RefreshCastbars = function()
    local gui_UF = gui_Castbar.unitFramesModule
    if not gui_UF then return end
...
_G.GravityUI_Castbars = gui_Castbar.castbars
