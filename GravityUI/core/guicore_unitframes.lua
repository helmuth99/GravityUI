...
---------------------------------------------------------------------------
local gui_UF = {}
ns.gui_UnitFrames = gui_UF

-- Frame references
gui_UF.frames = {}
gui_UF.castbars = {}
gui_UF.previewMode = {}
gui_UF.auraPreviewMode = {}  -- Tracks buff/debuff preview state (keyed by "unitKey_buff" or "unitKey_debuff")
...
local function GetDB()
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.guiUnitFrames then
        return guiCore.db.profile.guiUnitFrames
    end
    return nil
end
...
---------------------------------------------------------------------------
-- HELPER: Pixel-perfect scaling (uses guiCore:Scale if available)
---------------------------------------------------------------------------
local function Scale(x)
    if guiCore and guiCore.Scale then
        return guiCore:Scale(x)
    end
    return x
end
...
local function GetTexturePath(textureName)
    ...
    if not name or name == "" then
        local general = GetGeneralSettings()
        name = general and general.texture or "Gravity Normal"
    end
...
local function GetAbsorbTexturePath(textureName)
    ...
    return LSM:Fetch("statusbar", name) or "Interface\\AddOns\\GravityUI\\assets\\absorb_stripe"
end
---------------------------------------------------------------------------
...
local function GetUnitHostilityColor(unit)
    ...
    -- Default to neutral if we can't determine reaction
    return 0.5, 0.5, 0.5, 1
end
...
local function UpdateAbsorbs(frame)
    ...
    do
		...
		-- Gemeinsame Einstellungen für beide Balken
        local barWidth = frame.healthBar:GetWidth() / 2 
        local frameLevel = frame.healthBar:GetFrameLevel()

        ---------------------------------------------------------
        -- 1. ATTACHED BAR (Schild bei fehlendem Leben)
        ---------------------------------------------------------
        frame.absorbBar:ClearAllPoints()
        -- Fest RECHTS am Frame verankert
        ...
        frame.absorbBar:SetReverseFill(true) -- Wächst von Rechts nach Links
        frame.absorbBar:SetFrameLevel(frameLevel + 1)
        ...
        ---------------------------------------------------------
        -- 2. OVERFLOW BAR (Schild bei 100% Leben / Überlauf)
        ---------------------------------------------------------
        -- WICHTIG: Nur diesen einen Block für die OverflowBar nutzen!
        frame.absorbOverflowBar:ClearAllPoints()
        -- Ebenfalls fest RECHTS am Frame verankert
        frame.absorbOverflowBar:SetPoint("TOPRIGHT", frame.healthBar, "TOPRIGHT", 0, 0)
        frame.absorbOverflowBar:SetPoint("BOTTOMRIGHT", frame.healthBar, "BOTTOMRIGHT", 0, 0)
        frame.absorbOverflowBar:SetWidth(barWidth)
        
        frame.absorbOverflowBar:SetReverseFill(true) -- Wächst von Rechts nach Links
        -- HÖHERES Level (+2), damit sie ÜBER dem vollen Leben sichtbar ist
        frame.absorbOverflowBar:SetFrameLevel(frameLevel + 2) 
...
---------------------------------------------------------------------------
-- UPDATE: Stance/Aura text (player only: shows current form, stance, or aura)
---------------------------------------------------------------------------
local function UpdateStance(frame)
    ...
    local settings = GetUnitSettings("player")
    if not settings or not settings.stance or not settings.stance.enabled then
        frame.stanceText:Hide()
        if frame.stanceIcon then frame.stanceIcon:Hide() end
        return
    end

    local stanceSettings = settings.stance
    local general = GetGeneralSettings()
	local _, class = UnitClass("player")
	...
	if class == "HUNTER" or class == "DEATHKNIGHT" or class == "WARLOCK" then
		local specID = GetSpecialization()
		local specName = GetSpecializationInfo(specID)
		if PET_SPECS[specName] then
			isPetClass = true
		end
	end

	local isNoPet = isPetClass and not UnitExists("pet") and not IsMounted()
    -- Update font and position from current settings
    local fontPath = GetFontPath()
    local fontOutline = general and general.fontOutline or "OUTLINE"
	if isNoPet then
    local PetfontSize = stanceSettings.PetfontSize or 20
    frame.stanceText:SetFont(fontPath, PetfontSize, fontOutline)
	else
	...
	local petoffsetX = stanceSettings.petoffsetX or 0
    local petoffsetY = stanceSettings.petoffsetY or -2

    frame.stanceText:ClearAllPoints()
	if isNoPet then
	frame.stanceText:SetPoint(anchorInfo.point, frame, anchorInfo.point, petoffsetX, petoffsetY)
	else
	...
	-- "No Pet" Logik & Dynamische Farbe
		if isNoPet then
			local noPetText = stanceSettings.noPetText or "NO PET"
			frame.stanceText:SetText(noPetText)
			
			-- DYNAMISCHE FARBE: 
			-- Wir prüfen, ob es eine extra Farbe 'noPetColor' in deinen Settings gibt,
			-- ansonsten nehmen wir die 'customColor' der Stance-Einstellungen.
			if stanceSettings.useClassColor then
				local petcolor = RAID_CLASS_COLORS[class]
				if petcolor then
					frame.stanceText:SetTextColor(petcolor.r, petcolor.g, petcolor.b, 1)
				else
					frame.stanceText:SetTextColor(1, 1, 1, 1)
				end
			else
				local c = stanceSettings.noPetColor or { 1, 1, 1, 1 }
				frame.stanceText:SetTextColor(c[1], c[2], c[3], c[4] or 1)
			end
			
			frame.stanceText:Show()
			if frame.stanceIcon then frame.stanceIcon:Hide() end
			return -- "No Pet" Warnung hat Vorrang
		end

    -- Normal Stance/Form logic
...
local function CreateBossFrame(unit, frameKey, bossIndex)
    ...
    -- Create secure button with unique name (gui_Boss1, gui_Boss2, etc.)
    local frameName = "gui_Boss" .. bossIndex
    ...
    frame:HookScript("OnShow", function(self)
        ...
        if gui_UF.previewMode[bossKey] then return end
        UpdateFrame(self)
    end)
...
local function ForceUpdateToT()
    local totFrame = gui_UF.frames and gui_UF.frames.targettarget
...
local function CreateUnitFrame(unit, unitKey)
    ...
    local frameName = "gui_" .. unitKey:gsub("^%l", string.upper)
    local frame = CreateFrame("Button", frameName, UIParent, "SecureUnitButtonTemplate, BackdropTemplate, PingableUnitFrameTemplate")
    ...
    if unitKey == "player" then
		...
		frame:RegisterEvent("UPDATE_SHAPESHIFT_FORM") -- Stance/aura text
		frame:RegisterEvent("PLAYER_MOUNT_DISPLAY_CHANGED") -- Mount
    end
    ...
    frame:SetScript("OnEvent", function(self, event, arg1)
		...
		elseif event == "UPDATE_SHAPESHIFT_FORM" then
            -- Stance/aura text update (player only)
            if self.unitKey == "player" then
                UpdateStance(self)
            end	
        elseif event == "RAID_TARGET_UPDATE" then
...
local function CreateCastbar(unitFrame, unit, unitKey)
    ...
    if gui_Castbar then
        return gui_Castbar:CreateCastbar(unitFrame, unit, unitKey)
    end
    return nil
end
...
local function CreateBossCastbar(unitFrame, unit, bossIndex)
    -- Delegate to castbar module
    if gui_Castbar then
        return gui_Castbar:CreateBossCastbar(unitFrame, unit, bossIndex)			  
    end																		
    return nil
end
...
local function UpdateAuras(frame)
    ...
    local buffPreviewActive = gui_UF.auraPreviewMode[unitKey .. "_buff"]
    local debuffPreviewActive = gui_UF.auraPreviewMode[unitKey .. "_debuff"]
...
---------------------------------------------------------------------------
function gui_UF:ShowPreview(unitKey)
        ...
        for i = 1, 5 do
            ...
            if frame then
                ...
                if settings and settings.castbar and settings.castbar.previewMode then
                    ...
                    if castbar and gui_Castbar then
                        -- Trigger the castbar to show its preview
                        gui_Castbar:RefreshBossCastbar(castbar, bossKey, settings.castbar, frame)
                    end
                end
...
function gui_UF:HidePreview(unitKey)
...
---------------------------------------------------------------------------
function gui_UF:ShowAuraPreview(unitKey, auraType)
...
function gui_UF:ShowAuraPreviewForFrame(frame, unitKey, auraType)
    if not frame then return end
    -- Get settings
    local settings = GetUnitSettings(unitKey)
    local auraSettings = settings and settings.auras or {}
...
function gui_UF:HideAuraPreview(unitKey, auraType)
...
function gui_UF:HideAuraPreviewForFrame(frame, unitKey, auraType)
    if not frame then return end
    local isDebuff = (auraType == "debuff")
    local containerKey = isDebuff and "previewDebuffIcons" or "previewBuffIcons"
    local container = frame[containerKey]
...
---------------------------------------------------------------------------
function gui_UF:RefreshFrame(unitKey)
    ...
    if unitKey == "boss" then
        ...
        if guiCore and guiCore.GetHUDFrameLevel then
            bossFrameLevel = guiCore:GetHUDFrameLevel(bossLayerPriority)
        end
        ...
        for i = 1, 5 do
            ...
            if frame then
                ...
                local castbar = self.castbars[bossKey]
                if castbar and gui_Castbar and gui_Castbar.RefreshBossCastbar then
                    local castSettings = settings.castbar
                    if castSettings then
                        gui_Castbar:RefreshBossCastbar(castbar, bossKey, castSettings, frame)
                    end
                end
            end
        end
    ...
    if unitKey == "player" and settings.indicators then
        ...
        if frame.indicatorFrame then
            ...
            if guiCore and guiCore.GetHUDFrameLevel then
                local indicatorLevel = guiCore:GetHUDFrameLevel(indicatorPriority)
    ...
    if castSettings and castSettings.enabled then
        if castbar and gui_Castbar and gui_Castbar.RefreshCastbar then
            -- Castbar exists - refresh it
            gui_Castbar:RefreshCastbar(castbar, unitKey, castSettings, frame)
        elseif not castbar and gui_Castbar and gui_Castbar.CreateCastbar then
            -- Castbar doesn't exist but is now enabled - create it
            self.castbars[unitKey] = gui_Castbar:CreateCastbar(frame, unitKey, unitKey)																					   
        end
    end
end 

function gui_UF:RefreshAll()
...
---------------------------------------------------------------------------
gui_UF.editModeActive = false

-- Register slider references for real-time sync during edit mode
function gui_UF:RegisterEditModeSliders(unitKey, xSlider, ySlider)
...
function gui_UF:NotifyPositionChanged(unitKey, offsetX, offsetY)
...
local function CreateNudgeButton(parent, direction, deltaX, deltaY, unitKey)
    ...
    btn:SetScript("OnEnter", function(self)
        self.line1:SetColorTexture(0, 0.74901960784314, 1, 1)
        self.line2:SetColorTexture(0, 0.74901960784314, 1, 1)
    end)
    ...
    btn:SetScript("OnClick", function()
        ...
        if settings then
            ...
            gui_UF:NotifyPositionChanged(settingsKey, settings.offsetX, settings.offsetY)
        end
    end)
...
function gui_UF:EnableEditMode()
    if InCombatLockdown() then
        print("|cFF56D1FFGravityUI|r: Cannot enter Edit Mode during combat.")
        return
    end
    ...
    if not self.exitEditModeBtn then
        local exitBtn = CreateFrame("Button", "gui_ExitEditModeBtn", UIParent, "BackdropTemplate")
        ...
        exitBtn:SetScript("OnClick", function()
            if EditModeManagerFrame and EditModeManagerFrame:IsShown() then
                ...
                -- Fallback for /gui editmode case
                gui_UF:DisableEditMode()
            end
        end)
    ...
    for unitKey, frame in pairs(self.frames) do
        ...
        frame:SetScript("OnMouseDown", function(self, button)
            if button == "LeftButton" and gui_UF.editModeActive then
                local guiCore = _G.GravityUI and _G.GravityUI.guiCore
                if guiCore and guiCore.SelectEditModeElement then
                    guiCore:SelectEditModeElement("unitframe", self._editModeUnitKey)
                end
            end
        end)

        frame:SetScript("OnDragStart", function(self)
            if GUI_UF.editModeActive then
                ...
                self:SetScript("OnUpdate", function(self)
                    ...
                    if selfX and selfY and parentX and parentY then
                        ...
                        if settings then
                            ...
                            gui_UF:NotifyPositionChanged(settingsKey, offsetX, offsetY)
                        end
                    end
                end)
            end
        ...
        frame:SetScript("OnKeyDown", function(self, key)
            if not gui_UF.editModeActive then return end
    ...
    print("|cFF56D1FFGravityUI|r: Edit Mode |cff00ff00ENABLED|r - Drag frames to reposition.")
end

function gui_UF:DisableEditMode()
    self.editModeActive = false

    -- Clear Edit Mode selection (hides arrows on selected element)
    local guiCore = _G.GravityUI and _G.GravityUI.guiCore
    if guiCore and guiCore.ClearEditModeSelection then
        guiCore:ClearEditModeSelection()
    end
    ...
    print("|cFF56D1FFGravityUI|r: Edit Mode |cffff0000DISABLED|r - Positions saved.")
end

function gui_UF:ToggleEditMode()
...
function gui_UF:HideBlizzardFrames()
    local db = GetDB()
    ...
    -- Hide Blizzard Player Castbar if our gui player castbar is enabled
    if db.player and db.player.castbar and db.player.castbar.enabled then
        if PlayerCastingBarFrame then
            ...
            if not PlayerCastingBarFrame._guiShowHooked then
                PlayerCastingBarFrame._guiShowHooked = true
    ...
    -- Hide Focus frame visuals (always hide Blizzard focus frame when gui unit frames are enabled)									 
    ...
    if db.boss and db.boss.enabled then
        ...
        if BossTargetFrameContainer and not BossTargetFrameContainer._guiEditModeFixed then
            ...
            BossTargetFrameContainer._guiEditModeFixed = true
        end
    end
end
...
function gui_UF:Initialize()
    ...
    if gui_Castbar then
        ...
        gui_Castbar:SetUnitFramesModule(self)
        gui_Castbar.castbars = self.castbars
    end
...
---------------------------------------------------------------------------
-- Hook Blizzard's Edit Mode to trigger gui's edit mode when user enters via Esc > Edit Mode
function gui_UF:HookBlizzardEditMode()
    if not EditModeManagerFrame then return end
    if self._blizzEditModeHooked then return end
    self._blizzEditModeHooked = true

    -- Track if we triggered from Blizzard Edit Mode (vs /gui editmode)
    self.triggeredByBlizzEditMode = false

    hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
        if InCombatLockdown() then return end
        if self.editModeActive then return end  -- Already active via /gui editmode
    ...
    hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
        ...
        if not self.triggeredByBlizzEditMode then return end  -- Don't exit if user used /gui editmode
        self.triggeredByBlizzEditMode = false
        self:DisableEditMode()
    end)
end

-- Hide Blizzard's selection frames when gui frames are enabled
-- Called during EnableEditMode() to prevent visual conflicts
function gui_UF:HideBlizzardSelectionFrames()
    local function HideSelection(parent, unitKey)
        ...
        -- Hook OnShow to persistently hide while gui frames are enabled
        if not parent.Selection._guiHooked then
            parent.Selection._guiHooked = true
...
---------------------------------------------------------------------------
function gui_UF:RegisterWithClique()
...
initFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        C_Timer.After(0.5, function()
            ...
            gui_UF:HookBlizzardEditMode()
            -- Register frames with Clique after creation
            C_Timer.After(0.5, function()
                gui_UF:RegisterWithClique()
            end)
        end)
    ...
    elseif event == "PLAYER_REGEN_ENABLED" then
        ...
        if gui_UF.pendingInitialize then
            gui_UF.pendingInitialize = false
            gui_UF:Initialize()
            gui_UF:HookBlizzardEditMode()
            C_Timer.After(0.5, function()
                gui_UF:RegisterWithClique()
...
_G.GravityUI_RefreshAuras = function(unitKey)
    if unitKey then
        ...
        if unitKey == "boss" then
            ...
            local frame = gui_UF.frames[unitKey]
            if frame then
                lastAuraUpdate[unitKey] = 0
                UpdateAuras(frame)
            end
...
_G.GravityUI_ShowUnitFramePreview = function(unitKey)
    gui_UF:ShowPreview(unitKey)
end

_G.GravityUI_HideUnitFramePreview = function(unitKey)
    gui_UF:HidePreview(unitKey)							   
end

_G.GravityUI_ShowAuraPreview = function(unitKey, auraType)
    gui_UF:ShowAuraPreview(unitKey, auraType)
end

_G.GravityUI_HideAuraPreview = function(unitKey, auraType)
    gui_UF:HideAuraPreview(unitKey, auraType)
end

_G.GravityUI_ToggleUnitFrameEditMode = function()
    gui_UF:ToggleEditMode()
end

-- Register slider references for real-time sync during edit mode
_G.GravityUI_RegisterEditModeSliders = function(unitKey, xSlider, ySlider)
    gui_UF:RegisterEditModeSliders(unitKey, xSlider, ySlider)															 
end

-- Global references for external access
_G.GravityUI_UnitFrames = gui_UF.frames
_G.GravityUI_Castbars = gui_UF.castbars

-- Castbar functions are now in gui_castbar.lua module												  
...
local function GetAnchorFrame(anchorType)
    ...
    elseif anchorType == "primary" then
        local GUICore = _G.GravityUI and _G.GravityUI.GUICore
        return GUICore and GUICore.powerBar
    elseif anchorType == "secondary" then
        local GUICore = _G.GravityUI and _G.GravityUI.GUICore
        return GUICore and GUICore.secondaryPowerBar
    end
    return nil
end
...
_G.GravityUI_UpdateAnchoredUnitFrames = function()
    ...
    if playerAnchorType and playerAnchorType ~= "disabled" and gui_UF.frames.player then
        ...
        if anchorFrame and anchorFrame:IsShown() then
            ...
            if anchor then
                local frame = gui_UF.frames.player
    ...
    if targetAnchorType and targetAnchorType ~= "disabled" and gui_UF.frames.target then
        local anchorFrame = GetAnchorFrame(targetAnchorType)
        if anchorFrame and anchorFrame:IsShown() then
            local anchor = GetAnchorDimensions(anchorFrame, targetAnchorType)
            if anchor then
                local frame = gui_UF.frames.target
                local frameWidth = frame:GetWidth()
                local frameHeight = frame:GetHeight()
...
_G.GravityUI_UpdateCDMAnchoredUnitFrames = _G.GravityUI_UpdateAnchoredUnitFrames
...
_G.GravityUI_UpdateLockedCastbarToEssential = function(forceUpdate)
    local db = GetDB()
    ...
    if _G.GravityUI_RefreshCastbar then
        _G.GravityUI_RefreshCastbar("player")
...
-- Width is now controlled by dual anchors, so just refresh the castbar
_G.GravityUI_UpdateLockedCastbarToUtility = function(forceUpdate)
    local db = GetDB()
    if not db or not db.player then return end
...
_G.GravityUI_UpdateLockedCastbarToFrame = function()
