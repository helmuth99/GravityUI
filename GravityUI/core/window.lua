...
---------------------------------------------------------------------------
-- ELLESMEREUI BRIDGE PANEL ("Back to GravityUI" sidebar)
---------------------------------------------------------------------------
-- A slim panel that appears RIGHT next to the EllesmereUI options window.
-- Clicking it closes EllesmereUI and opens GravityUI.
--
-- POSITIONING:  EllesmereUIFrame has its own pixel-perfect scale, so direct
--   frame-relative anchors produce wrong offsets.  We convert EUI coords to
--   UIParent-space and anchor our panel absolutely via UIParent BOTTOMLEFT.
--
-- VERTICAL TEXT: WoW has no text-rotation API.  We stack each character of
--   "BACK TO GRAVITYUI" on its own line to simulate a 90-degree label.
--
-- LAZY FRAME:  EllesmereUIFrame is created on the FIRST Toggle() call.
--   We hook EllesmereUI.Toggle via hooksecurefunc and attach OnShow/OnHide
--   immediately after (the frame is guaranteed to exist at that point).
---------------------------------------------------------------------------
do
    local backPanel  = nil    -- our panel frame (created once)
    local euiHooked  = false  -- set true after OnShow/OnHide hooks are attached

    -- Panel dimensions (UIParent coordinate units, scale 1.0)
    local PANEL_W = 46
    local PANEL_H = 320

    -- -----------------------------------------------------------------------
    -- PositionPanel: anchor our panel directly TO euiFrame so it moves
    -- with EllesmereUI when the user drags the window.
    -- Side-detection uses GetRight() to decide LEFT vs RIGHT placement.
    -- -----------------------------------------------------------------------
    local function PositionPanel(euiFrame)
        if not backPanel then return end
        backPanel:ClearAllPoints()

        -- Always place panel on the LEFT side of EllesmereUI.
        -- EUI has 100 EUI-units of invisible padding on the left side
        -- (BG_WIDTH=1500, CLICK_W=1300 → 100px per side).
        -- We compensate so the panel sits flush with the VISIBLE left border.
        local euiScale = euiFrame:GetEffectiveScale()
        local uiScale  = UIParent:GetEffectiveScale()
        local euiPad   = math.floor(100 * euiScale / uiScale)

        -- RIGHT edge of our panel = visible left edge of EUI (1px gap)
        backPanel:SetPoint("RIGHT", euiFrame, "LEFT", euiPad - 1, 0)
        -- Vertical: center panel on EUI's midpoint
        backPanel:SetPoint("TOP", euiFrame, "CENTER", 0, PANEL_H / 2)
    end
    ...
    -- -----------------------------------------------------------------------
    local function CreateBackPanel()
        if backPanel then return end

        local C = GUI.Colors

        backPanel = CreateFrame("Frame", "GravityUI_EllesmereBackBar", UIParent, "BackdropTemplate")
        backPanel:SetSize(PANEL_W, PANEL_H)
        backPanel:SetFrameStrata("DIALOG")   -- same strata as EllesmereUIFrame
        backPanel:SetFrameLevel(105)
        backPanel:EnableMouse(true)
        backPanel:Hide()

        -- Glassmorphic background (matches GravityUI main window style)
        backPanel:SetBackdrop({
            bgFile   = "Interface\\Buttons\\WHITE8x8",
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = 1,
        })
        backPanel:SetBackdropColor(C.bgGlass[1], C.bgGlass[2], C.bgGlass[3], C.bgGlass[4] or 0.95)
        backPanel:SetBackdropBorderColor(C.border[1], C.border[2], C.border[3], 0.3)

        -- Accent stripe on the RIGHT edge (faces EllesmereUI)
        local accentStripe = backPanel:CreateTexture(nil, "OVERLAY")
        accentStripe:SetWidth(3)
        accentStripe:SetPoint("TOPRIGHT",    0, -10)
        accentStripe:SetPoint("BOTTOMRIGHT", 0,  10)
        accentStripe:SetColorTexture(C.accent[1], C.accent[2], C.accent[3], 1)

        -- GravityUI logo
        local logo = backPanel:CreateTexture(nil, "ARTWORK")
        logo:SetSize(30, 30)
        logo:SetPoint("TOP", backPanel, "TOP", 0, -16)
        logo:SetTexture(ns.ICON_PATH)

        -- "GravityUI" title below logo (bold via OUTLINE)
        local brandText = backPanel:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        GUI:SetFont(brandText, 9, "OUTLINE", C.accent)
        brandText:SetText("GravityUI")
        brandText:SetJustifyH("CENTER")
        brandText:SetPoint("TOP", logo, "BOTTOM", 0, -4)
        brandText:SetWidth(PANEL_W - 6)

        -- Thin separator
        local sep = backPanel:CreateTexture(nil, "OVERLAY")
        sep:SetHeight(1)
        sep:SetPoint("TOPLEFT",  backPanel, "TOPLEFT",  6, -64)
        sep:SetPoint("TOPRIGHT", backPanel, "TOPRIGHT", -6, -64)
        sep:SetColorTexture(1, 1, 1, 0.08)

        -- Arrow pointing RIGHT toward EllesmereUI (bold via OUTLINE)
        local arrow = backPanel:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        GUI:SetFont(arrow, 13, "OUTLINE", C.accent)
        arrow:SetText("→")
        arrow:SetPoint("TOP", sep, "BOTTOM", 0, -8)

        -- Vertical text (bold via OUTLINE): each character stacked top-to-bottom
        local vertLabel = backPanel:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        GUI:SetFont(vertLabel, 9, "OUTLINE", C.textMuted)
        vertLabel:SetText("B\nA\nC\nK\n \nT\nO\n \nG\nR\nA\nV\nI\nT\nY\nU\nI")
        vertLabel:SetJustifyH("CENTER")
        vertLabel:SetJustifyV("TOP")
        vertLabel:SetPoint("TOP",    backPanel, "TOP",    0, -88)
        vertLabel:SetPoint("BOTTOM", backPanel, "BOTTOM", 0,  10)
        vertLabel:SetWidth(PANEL_W - 6)

        -- Invisible full-panel click button
        local clickBtn = CreateFrame("Button", nil, backPanel)
        clickBtn:SetAllPoints(backPanel)
        clickBtn:SetFrameLevel(backPanel:GetFrameLevel() + 10)

        -- Hover: tint backdrop in accent color
        clickBtn:SetScript("OnEnter", function()
            backPanel:SetBackdropColor(C.accent[1] * 0.22, C.accent[2] * 0.22, C.accent[3] * 0.22, 0.92)
            backPanel:SetBackdropBorderColor(C.accent[1], C.accent[2], C.accent[3], 0.65)
            vertLabel:SetTextColor(C.accentLight[1], C.accentLight[2], C.accentLight[3], 1)
            arrow:SetTextColor(C.accentLight[1], C.accentLight[2], C.accentLight[3], 1)
            GameTooltip:SetOwner(clickBtn, "ANCHOR_TOPRIGHT")
            GameTooltip:AddLine("Back to GravityUI", C.accent[1], C.accent[2], C.accent[3])
            GameTooltip:AddLine("Close EllesmereUI and open GravityUI options.", 1, 1, 1)
            GameTooltip:Show()
        end)
        clickBtn:SetScript("OnLeave", function()
            backPanel:SetBackdropColor(C.bgGlass[1], C.bgGlass[2], C.bgGlass[3], C.bgGlass[4] or 0.96)
            backPanel:SetBackdropBorderColor(C.border[1], C.border[2], C.border[3], 0.3)
            vertLabel:SetTextColor(C.textMuted[1], C.textMuted[2], C.textMuted[3], 1)
            arrow:SetTextColor(C.accent[1], C.accent[2], C.accent[3], 1)
            GameTooltip:Hide()
        end)

        -- Click: close EllesmereUI → open GravityUI
        clickBtn:SetScript("OnClick", function()
            ...
            C_Timer.After(0.05, function() GUI:Show() end)
        end)
    end

    -- -----------------------------------------------------------------------
    -- Attach OnShow / OnHide hooks to the actual EllesmereUIFrame.
    -- Called once the frame has been lazily created (on first Toggle).
    -- -----------------------------------------------------------------------
    local function AttachFrameHooks()
        if euiHooked then return end
        local euiFrame = (_G.EllesmereUI and _G.EllesmereUI._mainFrame)
                      or _G["EllesmereUIFrame"]
        if not euiFrame or not euiFrame.HookScript then return end

        euiHooked = true

        -- If EllesmereUI is already visible right now, show the panel immediately
        if euiFrame:IsShown() then
            PositionPanel(euiFrame)
            backPanel:Show()
        end
        ...
        euiFrame:HookScript("OnHide", function()
            backPanel:Hide()
        end)
    end

    -- -----------------------------------------------------------------------
    -- Intercept every call to EllesmereUI:Toggle()
    -- EllesmereUIFrame is created inside Toggle (lazily), so by the time our
    -- hook runs the frame already exists.  We attach once and stop checking.
    -- -----------------------------------------------------------------------
    local function SetupToggleHook()
        local E = _G.EllesmereUI
        if not E or not E.Toggle then return false end

        -- Build the visual panel first (idempotent)
        CreateBackPanel()

        -- Hook Toggle so we capture every future open/close
        hooksecurefunc(E, "Toggle", function()
            -- Frame is now guaranteed to exist
            AttachFrameHooks()
        end)

        -- If the frame already exists (EUI was opened before our hook ran)
        AttachFrameHooks()
    ...
    -- -----------------------------------------------------------------------
    -- Bootstrap: try immediately, then on ADDON_LOADED as a safety net
    -- -----------------------------------------------------------------------
    local function Bootstrap()
        if SetupToggleHook() then return end

        -- EllesmereUI not yet loaded → wait for it
        ...
        watcher:SetScript("OnEvent", function(self, _, addonName)
            ...
            -- Defer one frame so EllesmereUI finishes its own init
            C_Timer.After(0.1, SetupToggleHook)
        end)
    end

    -- Run at PLAYER_LOGIN to be safe (fires after all ADDON_LOADED events)
