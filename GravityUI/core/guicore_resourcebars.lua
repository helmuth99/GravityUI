...
local function GetDefaultTexture()
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.general then
        return guiCore.db.profile.general.texture or "Gravity"
    end
...
function guiCore:RegisterPowerBarEditModeSliders(barKey, xSlider, ySlider)
...
function guiCore:NotifyPowerBarPositionChanged(barKey, offsetX, offsetY)
...
local function GetResourceColor(resource)
    ...
    local pc = guiCore and guiCore.db and guiCore.db.profile.powerColors
...
local function CreatePowerBarNudgeButton(parent, direction, deltaX, deltaY, barKey)
    ...
    btn:SetScript("OnClick", function()
        local cfg = (barKey == "primary") and guiCore.db.profile.powerBar or guiCore.db.profile.secondaryPowerBar
        ...
        guiCore:NotifyPowerBarPositionChanged(barKey, cfg.offsetX, cfg.offsetY)
    end)
...
function guiCore:EnablePowerBarEditMode()
            ...
            bar:SetScript("OnMouseDown", function(self, button)
                if button == "LeftButton" and PowerBarEditMode.active then
                    if guiCore and guiCore.SelectEditModeElement then
                        guiCore:SelectEditModeElement("powerbar", self._editModeBarKey)
                    end
                end
            end)
                ...
                if PowerBarEditMode.active then
                    ...
                    self:SetScript("OnUpdate", function(frame)
                        ...
                        if selfX and selfY and parentX and parentY then
                            ...
                            -- Notify options panel
                            guiCore:NotifyPowerBarPositionChanged(barKey, offsetX, offsetY)
                        end
                    end)
                end
            end)
...
function guiCore:DisablePowerBarEditMode()
...
function guiCore:UpdatePowerBar()
    local cfg = self.db.profile.powerBar
...
function guiCore:UpdatePowerBarTicks(bar, resource, max)
    local cfg = self.db.profile.powerBar
...
_G.GravityUI_UpdateLockedPowerBar = function()
    local guiCore = _G.GravityUI and _G.GravityUI.guiCore
    if not guiCore or not guiCore.db then return end

    local cfg = guiCore.db.profile.powerBar
...
_G.GravityUI_UpdateLockedPowerBarToUtility = function()
    local guiCore = _G.GravityUI and _G.GravityUI.guiCore
    if not guiCore or not guiCore.db then return end
...
_G.GravityUI_UpdateLockedSecondaryPowerBar = function()
    local guiCore = _G.GravityUI and _G.GravityUI.guiCore
    if not guiCore or not guiCore.db then return end

    local cfg = guiCore.db.profile.secondaryPowerBar
    if not cfg.enabled or not cfg.lockedToEssential then return end
...
_G.GravityUI_UpdateLockedSecondaryPowerBarToUtility = function()
    local guiCore = _G.GravityUI and _G.GravityUI.guiCore
    if not guiCore or not guiCore.db then return end
...
function guiCore:GetSecondaryPowerBar()
...
function guiCore:CreateFragmentedPowerBars(bar, resource, isVertical)
    local cfg = self.db.profile.secondaryPowerBar
...
function guiCore:UpdateFragmentedPowerDisplay(bar, resource, isVertical)
    local cfg = self.db.profile.secondaryPowerBar
    local maxPower = UnitPowerMax("player", resource)
...
function guiCore:UpdateSecondaryPowerBarTicks(bar, resource, max)
    local cfg = self.db.profile.secondaryPowerBar
...
function guiCore:UpdateSecondaryPowerBar()
    local cfg = self.db.profile.secondaryPowerBar
    if not cfg.enabled then
...
function guiCore:OnUnitPower(_, unit)
...
local oldRefreshAll = guiCore.RefreshAll
...
function guiCore:OnRunePowerUpdate()
    local now = GetTime()
...
local function InitializeResourceBars(self)
    ...
    C_Timer.After(0.6, function()
        if EditModeManagerFrame and not guiCore._powerBarEditModeHooked then
            guiCore._powerBarEditModeHooked = true
            hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
                if not InCombatLockdown() then
                    guiCore:EnablePowerBarEditMode()
                end
            end)
            hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
                if not InCombatLockdown() then
                    guiCore:DisablePowerBarEditMode()
                end
            end)
        end
    end)
...
function guiCore:OnSpecChanged()
...
function guiCore:OnShapeshiftChanged()
    -- Druid form changes affect primary/secondary resources
    self:UpdatePowerBar()
    self:UpdateSecondaryPowerBar()
end
-- Hook into that shit
local oldOnEnable = guiCore.OnEnable
