...
function guiCore:ImportProfileFromString(str)
    if not self.db or not self.db.profile then
...
function guiCore.SafeSetBackdrop(frame, backdropInfo, borderColor)
    ...
    if not hasValidSize then
        frame.__guiBackdropPending = backdropInfo
        frame.__guiBackdropBorderColor = borderColor
        guiCore.__pendingBackdrops = guiCore.__pendingBackdrops or {}
        guiCore.__pendingBackdrops[frame] = true

        -- Set up deferred processing via OnUpdate (for when dimensions become valid)
        if not guiCore.__backdropUpdateFrame then
            ...
            updateFrame:SetScript("OnUpdate", function(self, delta)
                ...
                local processed = {}
                for pendingFrame in pairs(guiCore.__pendingBackdrops or {}) do
                    if pendingFrame and pendingFrame.__guiBackdropPending ~= nil then
                        ...
                        if checkOk and checkResult and not InCombatLockdown() then
                            local setOk = pcall(pendingFrame.SetBackdrop, pendingFrame, pendingFrame.__guiBackdropPending)
                            if setOk and pendingFrame.__guiBackdropPending and pendingFrame.__guiBackdropBorderColor then
                                local c = pendingFrame.__guiBackdropBorderColor
                                pendingFrame:SetBackdropBorderColor(c[1], c[2], c[3], c[4] or 1)
                            end
                            pendingFrame.__guiBackdropPending = nil
                            pendingFrame.__guiBackdropBorderColor = nil
                ...
                for _, pf in ipairs(processed) do
                    guiCore.__pendingBackdrops[pf] = nil
                end

                -- Stop OnUpdate if no more pending
                local hasAny = false
                for _ in pairs(guiCore.__pendingBackdrops or {}) do
                    hasAny = true
                    break
                end
                if not hasAny then
            ...
            guiCore.__backdropUpdateFrame = updateFrame
        end
        guiCore.__backdropUpdateFrame:Show()
        return false
    end

    -- If in combat, defer backdrop setup to avoid secret value errors
    if InCombatLockdown() then
        local alreadyPending = guiCore.__pendingBackdrops and guiCore.__pendingBackdrops[frame]
        if not alreadyPending then
            frame.__guiBackdropPending = backdropInfo
            frame.__guiBackdropBorderColor = borderColor

            if not guiCore.__backdropEventFrame then
                ...
                eventFrame:SetScript("OnEvent", function(self)
                    ...
                    guiCore.__pendingBackdrops = {}
                end)
                guiCore.__backdropEventFrame = eventFrame
            end

            guiCore.__pendingBackdrops = guiCore.__pendingBackdrops or {}
            guiCore.__pendingBackdrops[frame] = true
            guiCore.__backdropEventFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
        end
        return false
    end
...
-- EssentialCooldownViewer/UtilityCooldownViewer → gui_ncdm.lua
-- BuffIconCooldownViewer/BuffBarCooldownViewer → gui_buffbar.lua
guiCore.viewers = {
    -- "EssentialCooldownViewer",  -- Handled by NCDM
    -- "UtilityCooldownViewer",    -- Handled by NCDM
    -- "BuffIconCooldownViewer",   -- Handled by gui_buffbar.lua
}
...
local defaults = {
    profile = {
        ...
        general = {
            ...
            font = "Gravity",  -- Default font face
            fontOutline = "OUTLINE",  -- Default font outline: "", "OUTLINE", "THICKOUTLINE"
            texture = "Gravity v5",  -- Default bar texture
            darkMode = false,
			...
			chatBubbleFontSize = 8, -- Chatbubble Fontsize
			chatBubbleFontOutline = "OUTLINE", -- Chatbubble Font Ouline
            ...
            addGravityUIButton = true,  -- Add Gravity UI button to ESC menu (opt-in)
			gameMenuFontSize = 14, -- Game menu button font size																 
            ...
            skinRollFrames = false,  -- Skin loot roll frames
            skinRollSpacing = 6,  -- Spacing between roll frames
            skinUseClassColor = true,  -- Use class color for skin accents
            skinCustomColor = { 0, 0.74901960784314, 1, 1 },  -- Custom skin accent color
            -- QoL Automation
            sellJunk = true,
        ...
        mplusTimer = {
            ...
            showAffixes = true,
            showObjectives = true,
            position = { point = "TOPRIGHT", relPoint = "TOPRIGHT", x = -11.667, y = -204.998 },
        },
        ...
        character = {
            ...
            headerColor = {0, 0.74901960784314, 1},  -- Header color (default accent/blue, used when headerClassColor is off)
        ...
        -- FPS Settings Backup (stores user's CVars before applying Gravity's settings)
        fpsBackup = nil,

        -- gui New Cooldown Display Manager (NCDM)
        ...
        ncdm = {
            ...
            trackedBar = {
                ...
                texture = "Gravity v5",
                useClassColor = true,
                barColor = {0, 0.74901960784314, 1, 1},  -- blue accent fallback
                borderSize = 2,
                bgOpacity = 0.5,
                textSize = 14,
                spacing = 2,
        ...
        viewers = {
            EssentialCooldownViewer = {
                ...
                rotationHelperColor = { 0, 0.74901960784314, 1, 1 },  -- blue border
            ...
            -- BuffIconCooldownViewer removed - now handled by gui_buffbar.lua
        ...
        combatTimer = {
            ...
            font = "Gravity",       -- Font name (from LibSharedMedia)
		...
		-- DeleteFix (Insert delete automatic):
        DeleteFix = {
            enableDeleteFix = true, -- Standardmäßig eingeschaltet
        },
		
		-- Raid Marks (Marker Bar)
		marks = {
            enabled = false,
            mouseover = false,
            size = 24,
...
function guiCore:OnInitialize()
    self.db = LibStub("AceDB-3.0"):New("GravityUIDB", defaults, true)
    gui.db = self.db  -- Make database accessible to other gui modules
    ...
    -- Note: Main /gui command is handled by init.lua
    self:RegisterChatCommand("guicorerefresh", "ForceRefreshBuffIcons")
...
function guiCore:OnProfileChanged(event, db, profileKey)
    ...
    local function ApplyUIScale(scale)
        if InCombatLockdown() then
            guiCore._pendingUIScale = scale
            if not guiCore._scaleRegenFrame then
                guiCore._scaleRegenFrame = CreateFrame("Frame")
                guiCore._scaleRegenFrame:SetScript("OnEvent", function(self)
                    self:UnregisterEvent("PLAYER_REGEN_ENABLED")
                    if guiCore._pendingUIScale and not InCombatLockdown() then
                        UIParent:SetScale(guiCore._pendingUIScale)
                        guiCore._pendingUIScale = nil
                        if guiCore.UIMult then
                            guiCore:UIMult()
                        end
                    end
                end)
            end
            guiCore._scaleRegenFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
        else
            ...
            if not success then
                ...
                if not guiCore._scaleRegenFrame then
                    ...
                    guiCore._scaleRegenFrame:SetScript("OnEvent", function(self)
                        self:UnregisterEvent("PLAYER_REGEN_ENABLED")
                        if guiCore._pendingUIScale and not InCombatLockdown() then
                            pcall(function() UIParent:SetScale(guiCore._pendingUIScale) end)
                            guiCore._pendingUIScale = nil
                            if guiCore.UIMult then
                                guiCore:UIMult()
                            end
    ...
    if guiCore.Minimap then
        -- Small delay to ensure profile data is fully loaded
        C_Timer.After(0.1, function()
            if guiCore.Minimap.Refresh then
    ...
    if _G.GravityUI_RefreshSpecProfilesTab then
        _G.GravityUI_RefreshSpecProfilesTab()
    end
...
function guiCore:ShowProfileChangeNotification()
    -- Create a simple popup frame if it doesn't exist
    if not self.profileChangePopup then
        local popup = CreateFrame("Frame", "guiCore_ProfileChangePopup", UIParent, "BackdropTemplate")
...
function guiCore:SelectEditModeElement(elementType, elementKey)
...
function guiCore:ClearEditModeSelection()
    self:HideCurrentSelectionArrows()
...
function guiCore:HideCurrentSelectionArrows()
    ...
    if sel.selectedType == "unitframe" then
        ...
        if ns.gui_UnitFrames and ns.gui_UnitFrames.frames then
            local frame = ns.gui_UnitFrames.frames[sel.selectedKey]
...
function guiCore:ShowSelectionArrows(elementType, elementKey)
    if elementType == "unitframe" then
        if ns.gui_UnitFrames and ns.gui_UnitFrames.frames then
            local frame = ns.gui_UnitFrames.frames[elementKey]
            if frame and frame.editOverlay then
...
function guiCore:ShowNudgeButtons(overlay)
...
function guiCore:HideNudgeButtons(overlay)
    if not overlay then return end
...
function guiCore:OnEnable()
    ...
    C_Timer.After(1.0, function()
        ...
        if _G.GravityUI_RefreshUIHider then
            _G.GravityUI_RefreshUIHider()
        end
...
function guiCore:OpenConfig()
    -- Open the new custom GUI instead of AceConfig
    if GravityUI and GravityUI.GUI then
        GravityUI.GUI:Toggle()
    end
end

function guiCore:CreateMinimapButton()
    ...
    local dataObj = LDB:NewDataObject(ADDON_NAME, {
        type = "launcher",
        icon = "Interface\\AddOns\\GravityUI\\assets\\GravityLogo.tga",
        label = "Gravity UI",
        ...
        OnTooltipShow = function(tooltip)
            tooltip:SetText("|cFF30D1FFGravity UI|r")
...
function guiCore:SkinIcon(icon, settings)
...
function guiCore:SkinAllIconsInViewer(viewer)
    ...
    for _, icon in ipairs(children) do
        if IsCooldownIconFrame(icon) and (icon.icon or icon.Icon) then
            ...
            if not ok then
                ...
                print("|cffff4444[guiCore] SkinIcon error for", name, "icon:", err, "|r")
            end
        end
    end
end
...
function guiCore:ApplyViewerLayout(viewer)
    if not viewer or not viewer.GetName then return end
    local name     = viewer:GetName()
    local settings = self.db.profile.viewers[name]
    if not settings or not settings.enabled then return end
...
function guiCore:RescanViewer(viewer)
    if not viewer or not viewer.GetName then return end
    local name     = viewer:GetName()
    local settings = self.db.profile.viewers[name]
    if not settings or not settings.enabled then return end
...
function guiCore:ApplyViewerSkin(viewer)
    if not viewer or not viewer.GetName then return end
    local name     = viewer:GetName()
    local settings = self.db.profile.viewers[name]
    if not settings or not settings.enabled then return end
...
function guiCore:ProcessPendingIcons()
...
function guiCore:ForceRefreshBuffIcons()
...
function guiCore:ForceReskinAllViewers()
    for _, name in ipairs(self.viewers) do
        local viewer = _G[name]
        if viewer then
            local container = viewer.viewerFrame or viewer
...
function guiCore:HookEditMode()
    ...
    if EditModeManagerFrame then
        ...
        hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
            ...
            if BossTargetFrameContainer and not BossTargetFrameContainer._guiScaledSidesHooked then
                if BossTargetFrameContainer.GetScaledSelectionSides then
                    ...
                    BossTargetFrameContainer._guiScaledSidesHooked = true
                end
            end
        end)
        ...
        hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
            C_Timer.After(0.1, function()
                ...
                C_Timer.After(0.15, function()
                    for _, barName in ipairs({"GravityUIPrimaryPowerBar", "GravityUISecondaryPowerBar"}) do
...
function guiCore:ProcessPendingBackdrops()
...
function gui:GetGlobalFont()
    local LSM = LibStub("LibSharedMedia-3.0")
    local fontName = "Gravity"  -- Default fallback

    -- Read font from user settings
    if guiCore and guiCore.db and guiCore.db.profile and guiCore.db.profile.general then
        fontName = guiCore.db.profile.general.font or fontName
    end
    return LSM:Fetch("font", fontName) or [[Interface\AddOns\GravityUI\assets\Gravity.ttf]]
end

function gui:GetGlobalTexture()
    local LSM = LibStub("LibSharedMedia-3.0")
    -- For now, return Gravity texture. Will be configurable in General Tab (Feature #3)
    local textureName = "Gravity"
    return LSM:Fetch("statusbar", textureName) or "Interface\\AddOns\\GravityUI\\assets\\Gravity"
end

function gui:GetSkinColor()
    local db = gui.db and gui.db.profile
    if not db or not db.general then
        return 0, 0.74901960784314, 1, 1  -- Fallback to blue
    end
...
function guiCore:SafeSetFont(fontString, fontPath, size, flags)
...
-- Fallback to bundled Gravity font (always available, loaded early in media.lua)
local Gravity_FONT_PATH = [[Interface\AddOns\GravityUI\assets\Gravity.ttf]]
...
local function GetGlobalFontPath()
    if not guiCore.db or not guiCore.db.profile or not guiCore.db.profile.general then
        return Gravity_FONT_PATH
    end
    local fontName = guiCore.db.profile.general.font or "Gravity"
    local fontPath = LSM:Fetch("font", fontName)
    return fontPath or Gravity_FONT_PATH
end
...
local function ScheduleGlobalFontApply()
    ...
    C_Timer.After(0.05, function()
        ...
        if guiCore.ApplyGlobalFont then
            guiCore:ApplyGlobalFont()
        end
    end)
end

function guiCore:ApplyGlobalFont()
